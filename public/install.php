...
if(is_file($lock)||$databaseAlreadyInstalled){
    ...
    exit(
        ...
        '<p>Atualizações futuras devem ser aplicadas em <strong>Administração → Sistema</strong>.</p>'.
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    try{
        ...
        Database::execute(
            ...
            [$orgId,$email,$name,$email,password_hash($password,PASSWORD_DEFAULT),Clock::sql(),Clock::sql()]
        );
...
<p class="text-secondary">Produção: lms.tecnodataeducacional.com.br · America/Sao_Paulo.</p>
<?php if($error):?><div class="alert alert-danger"><?=e($error)?></div><?php endif;?>
<div class="requirement-list mb-4"><?php foreach($requirements as $label=>$ok):?><div class="requirement <?=$ok?'ok':'bad'?>"><span><?=$ok?'✓':'×'?></span> <?=e($label)?></div><?php endforeach;?></div>
<form method="post">
<?=Csrf::field()?>
<div class="mb-3"><label class="form-label">Nome do Super Administrador</label><input class="form-control" name="name" required></div>
<div class="mb-3"><label class="form-label">E-mail</label><input class="form-control" type="email" name="email" required></div>
<div class="mb-4"><label class="form-label">Senha</label><input class="form-control" type="password" name="password" minlength="10" required><div class="form-text">Mínimo de 10 caracteres.</div></div>
<button class="btn btn-brand w-100">Instalar estrutura e criar administrador</button>
</form>
</main>
</body>
</html>
