...
use Tecnodata\CRM\Core\Auth;use Tecnodata\CRM\Core\DB;use Tecnodata\CRM\Core\Security;use Tecnodata\CRM\Services\CollectionService;
Auth::requireLogin();if(!Auth::can('collector','supervisor','admin')){http_response_code(403);exit('Sem acesso');}
$u=Auth::user();$id=(int)($_GET['id']??0);
$c=DB::fetch("SELECT c.*,m.last_purchase_at,m.days_without_purchase FROM clients c LEFT JOIN client_metrics m ON m.client_id=c.id WHERE c.id=?",[$id]);if(!$c){http_response_code(404);exit('Cliente não encontrado.');}
$state=CollectionService::debtState($id,(string)$c['omie_code']);
$debtSellerCode=CollectionService::debtSellerCode((string)$c['omie_code']);
$debtSeller=$debtSellerCode?DB::fetch("SELECT name FROM sellers WHERE omie_code=?",[$debtSellerCode]):null;
$debtSellerLabel=$debtSellerCode?($debtSeller['name']??$debtSellerCode):'Não informado na dívida';
$msg='';$err='';
...
$debts=$hasFinSeller
 ? DB::all("SELECT fm.*,s.name debt_seller_name FROM financial_movements fm INNER JOIN financial_accounts fa ON fa.omie_code=fm.account_omie_code AND fa.selected=1 AND fa.active=1 LEFT JOIN sellers s ON s.omie_code=fm.seller_omie_code WHERE fm.client_omie_code=? AND UPPER(fm.status) IN('ATRASADO','PAGTO_PARCIAL') ORDER BY fm.due_date",[$c['omie_code']])
 : DB::all("SELECT fm.*,NULL debt_seller_name FROM financial_movements fm INNER JOIN financial_accounts fa ON fa.omie_code=fm.account_omie_code AND fa.selected=1 AND fa.active=1 WHERE fm.client_omie_code=? AND UPPER(fm.status) IN('ATRASADO','PAGTO_PARCIAL') ORDER BY fm.due_date",[$c['omie_code']]);
$settled=$state['effective_debt']<=0.009;
include '_layout.php';?>
<?php if($msg):?><div class="alert alert-success alert-modern"><i class="fa-solid fa-circle-check"></i><?=$msg?></div><?php endif;?><?php if($err):?><div class="alert alert-danger alert-modern"><i class="fa-solid fa-triangle-exclamation"></i><?=e($err)?></div><?php endif;?>
...
<form method="post" class="mt-3"><input type="hidden" name="_token" value="<?=Security::csrf()?>">
<label class="form-label">Canal</label><div class="collection-channel-grid mb-3"><?php foreach(['ligacao'=>['fa-phone','Ligação'],'whatsapp'=>['fa-brands fa-whatsapp','WhatsApp'],'email'=>['fa-envelope','E-mail'],'outro'=>['fa-ellipsis','Outro']] as $v=>$a):?><label><input class="btn-check" type="radio" name="channel" value="<?=$v?>" <?=$v==='ligacao'?'checked':''?>><span><i class="fa-solid <?=$a[0]?>"></i><?=$a[1]?></span></label><?php endforeach;?></div>
<div class="mb-3"><label class="form-label">Resultado</label><select class="form-select" name="result" id="collectionResult"><option value="falou">Falou com o cliente</option><option value="nao_atendeu">Não atendeu</option><option value="sem_previsao">Sem previsão de pagamento</option><option value="promessa">Promessa de pagamento</option><option value="acordo">Acordo realizado</option><option value="pagamento">Pagamento recebido</option></select></div>
<div class="row g-2"><div class="col-6 mb-3"><label class="form-label">Valor recebido</label><input class="form-control" type="number" min="0" max="<?=$state['effective_debt']?>" step="0.01" name="amount" id="collectionAmount" value="0"></div><div class="col-6 mb-3"><label class="form-label">Promessa para</label><input class="form-control" type="date" name="promised_for"></div></div>
...
<div class="mb-3"><label class="form-label">Anotação</label><textarea class="form-control" rows="4" name="notes" placeholder="Resumo objetivo da conversa ou negociação"></textarea></div>
