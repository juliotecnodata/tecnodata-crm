...
if($kind==='sales'){
 ...
 $row=DB::fetch("SELECT ca.*,c.name client_name FROM collection_actions ca JOIN clients c ON c.id=ca.client_id WHERE ca.id=? AND ca.deleted_at IS NULL",[$id]);
}
 ...
 if(!Security::verifyCsrf($_POST['_token']??null))$err='Sessão expirada.';
   ...
   if($kind==='sales'){
      ...
      CollectionService::updateAction($id,(int)$u['id'],(string)($_POST['channel']??'ligacao'),(string)($_POST['result']??'falou'),
          (float)str_replace(',','.',(string)($_POST['amount']??0)),($_POST['promised_for']??'')?:null,trim((string)($_POST['notes']??'')));
   }
