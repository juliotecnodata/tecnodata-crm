...
elseif($finance==='clear')$where[]='COALESCE(m.overdue_amount,0)<=0';
...
try{
 ...
 foreach($rows as $r){
   ...
   $portfolio='<strong>'.e($sellerName).'</strong>';
   $sourceBadge=$r['assignment_id']?'<span class="signal-badge signal-attended">Definida no mês</span>':'<span class="signal-badge signal-muted">Base Omie</span>';
   $financeHtml=(float)$r['overdue_amount']>0
     ? '<span class="status-pill status-overdue">'.money($r['overdue_amount']).' vencido</span>'
