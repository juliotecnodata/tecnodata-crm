...
function client_central_crm_accessible(int $clientId): bool{
 ...
 return (bool)(DB::scalar("SELECT COUNT(*) FROM clients c
  WHERE c.id=? AND c.active=1 AND c.crm_inactive=0
    AND EXISTS (SELECT 1 FROM crm_account_links global_link
                JOIN crm_accounts global_account ON global_account.omie_code=global_link.crm_account_code AND global_account.active=1
                WHERE global_link.client_id=c.id)",[$clientId])??0);
}
...
$renderClients=function(bool $portfolioOnly=false,?string $forcedSegment=null){
 ...
 if($sellerFilter==='__none__'){
  ...
  if($crmPortfolioReady&&str_starts_with($sellerFilter,'crm:')){$w[]='c.crm_owner_user_id=?';$p[]=(int)substr($sellerFilter,4);}
  elseif(!$crmPortfolioReady){$w[]='('.$effectiveSellerSql.')=?';$p[]=$sellerFilter;}
 }
 ...
 $summaryWithoutResponsible=$crmPortfolioReady?"c.crm_owner_user_id IS NULL":"(".$summaryEffective.") IS NULL OR TRIM((".$summaryEffective."))=''";
 $summaryDivergence=$crmPortfolioReady?"(c.crm_owner_omie_code IS NOT NULL AND c.crm_owner_user_id IS NULL)":"COALESCE(c.seller_omie_code,'')<>COALESCE(c.omie_seller_code,'')";
 $summary=DB::one(
  "SELECT COUNT(*) total_clients,
          ...
          SUM(CASE WHEN ".$summaryWithoutResponsible." THEN 1 ELSE 0 END) without_seller,
          SUM(CASE WHEN ".$summaryDivergence." THEN 1 ELSE 0 END) seller_divergences,
          SUM(CASE WHEN ".($crmPortfolioReady?"0=1":"pa_summary.id IS NOT NULL")." THEN 1 ELSE 0 END) monthly_overrides,
...
$router->get('/api/client-quick-view',function(){
 ...
 try{
  ...
  if($accountCode!==''){
   $account=DB::one("SELECT a.*,cu.name owner_name,cu.email owner_email
                     FROM crm_accounts a
                     LEFT JOIN crm_users cu ON cu.omie_code=a.crm_user_code
                     WHERE a.omie_code=? AND a.active=1 LIMIT 1",[$accountCode]);
   if(!$account)json_response(['ok'=>false,'error'=>'Conta CRM não encontrada.'],404);
   $link=DB::one("SELECT client_id FROM crm_account_links WHERE crm_account_code=? ORDER BY is_primary DESC,updated_at DESC LIMIT 1",[$accountCode]);
   $clientId=(int)($link['client_id']??0);
  }elseif($clientId>0){
   $linkedCode=trim((string)(DB::scalar("SELECT l.crm_account_code
     FROM crm_account_links l
     JOIN crm_accounts a ON a.omie_code=l.crm_account_code AND a.active=1
     WHERE l.client_id=? ORDER BY l.is_primary DESC,l.updated_at DESC LIMIT 1",[$clientId])??''));
   if($linkedCode!==''){
  ...
  if($clientId>0){
   $client=DB::one("SELECT c.*,m.first_purchase_at,m.last_purchase_at,m.revenue_12m,m.orders_12m,m.avg_ticket_12m,m.avg_interval_days
                    ...
                    WHERE c.id=? LIMIT 1",[$clientId]);
  }

  $isAdmin=$role==='admin';$isSupervisor=$role==='supervisor';
  if(in_array($role,['seller','supervisor'],true)){
   $allowed=$accountCode!==''?CommercialAccountService::canView($u,$accountCode):($clientId>0&&client_central_crm_accessible($clientId));
   if(!$allowed)json_response(['ok'=>false,'error'=>'Este cadastro não pertence à base CRM ativa.'],403);
  ...
  if($client&&!$isAdmin&&(!empty($client['crm_inactive'])||empty($client['active'])))json_response(['ok'=>false,'error'=>'Cliente indisponível para consulta.'],404);

  $profile=['is_cfc'=>0,'is_reseller'=>0,'strategic_notes'=>''];
  if($accountCode!==''){
   try{$profile=CommercialAccountService::profile($accountCode);}catch(Throwable $ignored){}
  }elseif($clientId>0){
   try{$profile=CommercialClassificationService::get($clientId);}catch(Throwable $ignored){}
  }

  $contacts=[];$activities=[];$notes=[];
  if($accountCode!==''){
   try{$contacts=CommercialAccountService::contacts($accountCode);}catch(Throwable $ignored){}
   try{$activities=CommercialActivityService::history($accountCode,12);}catch(Throwable $ignored){}
   try{$notes=CommercialAccountService::notes($accountCode,5);}catch(Throwable $ignored){}
  }elseif($clientId>0){
   try{$activities=DB::all("SELECT a.*,u.name user_name,NULL return_task_id,NULL return_task_status,NULL return_due_at
                            FROM activities a JOIN users u ON u.id=a.user_id
                            WHERE a.client_id=? ORDER BY a.created_at DESC,a.id DESC LIMIT 12",[$clientId]);}catch(Throwable $ignored){}
  }
  ...
  $ownerName=trim((string)($account['owner_name']??''));
  if($ownerName===''&&!empty($client['crm_owner_user_id'])){
   try{$ownerName=trim((string)(DB::scalar("SELECT name FROM users WHERE id=?",[(int)$client['crm_owner_user_id']])??''));}catch(Throwable $ignored){}
  }
  if($ownerName==='')$ownerName='Sem responsável';

  $primaryContact=null;
  foreach($contacts as $contact){
   if(!$primaryContact||!empty($contact['mobile'])||!empty($contact['phone'])||!empty($contact['email'])){
    $primaryContact=$contact;if(!empty($contact['mobile']))break;
   }
  }
  ...
  $canWork=$isAdmin||(in_array($role,['seller','supervisor'],true)&&$accountCode!==''&&CommercialAccountService::canWork($u,$accountCode));
  $strategicNotes=trim((string)($profile['strategic_notes']??''));
  if($strategicNotes===''&&!empty($notes[0]['note']))$strategicNotes=trim((string)$notes[0]['note']);
  ...
  $editForm=null;$sellerOptions=[];
  if($client){
   try{$editForm=ClientService::formFromClient($client);}catch(Throwable $ignored){}
   try{$sellerOptions=DB::all("SELECT omie_code,name FROM sellers WHERE active=1 ORDER BY name");}catch(Throwable $ignored){}
  }
  ...
  error_log('[client-quick-view] '.$e->getMessage().' @ '.$e->getFile().':'.$e->getLine());
  json_response(['ok'=>false,'error'=>'Não foi possível carregar a ficha: '.$e->getMessage()],500);
 }
});
...
$router->get('/clients',function()use($renderClients){
 ...
 if(in_array($role,['admin','supervisor','seller'],true)){
  CommercialSchema::ensure();$filters=$_GET;$filters['scope']='all';$filters['_global_crm']=1;$filters['sort']='name';
  ...
  render('commercial_portfolio',[
   ...
   'owners'=>CommercialAccountService::owners(false),
...
$renderCommercialPortfolio=function(){
 ...
 $flash=$_SESSION['commercial_flash']??null;unset($_SESSION['commercial_flash']);
 render('commercial_portfolio',[
  'portfolio'=>$data,'owners'=>in_array((string)$u['role'],['admin','supervisor'],true)?CommercialAccountService::owners((string)$u['role']==='supervisor'):[],
...
$router->get('/commercial/accounts/{code}',function($p){
 ...
 render('commercial_account',[
  ...
  'audit'=>CommercialAccountService::audit($code),'linkAudit'=>CommercialAccountService::linkAudit($code),'canWork'=>$canWork,'canMaintain'=>$canMaintain,'backUrl'=>$backUrl,'backLabel'=>$backLabel,'flash'=>$flash,
...
$router->get('/api/commercial/accounts/{code}/link-candidates',function($p){
 ...
 if(!CommercialAccountService::canView($u,$code))json_response(['items'=>[],'message'=>'Esta Conta CRM não pertence à base ativa.'],403);
...
$router->post('/commercial/accounts/{code}/activity',function($p){
 ...
 if($returnTo==='home'){
  ...
  if($queryRaw!==''){parse_str($queryRaw,$parsed);foreach(['home_q','home_type','home_order','home_per_page','home_page'] as $key)if(isset($parsed[$key])&&!is_array($parsed[$key]))$safe[$key]=(string)$parsed[$key];}
  redirect('/'.($safe?'?'.http_build_query($safe):''));
 }
...
$router->get('/clients-sync',function(){
 ...
 if($syncRole==='supervisor'){$crmScope=commercial_active_crm_link_sql('c');$where[]=$crmScope;$summaryWhere[]=$crmScope;}
...
$router->post('/api/clients-sync/bulk',function(){
 ...
 try{
  ...
  if((string)(Auth::user()['role']??'')==='supervisor')$where[]=commercial_active_crm_link_sql('c');
  if($q!==''){[$searchSql,$searchParams]=crm_search_filter($q,array_merge(client_search_fields('c'),['ps.name','os.name']));if($searchSql!==''){$where[]=$searchSql;array_push($params,...$searchParams);}}
...
$router->get('/clients/{id}/edit',function($p){
 ...
 if(!$client||(in_array((string)($u['role']??''),['seller','supervisor'],true)&&!client_central_crm_accessible($id))){http_response_code(404);exit('Cliente não encontrado.');}
...
$router->post('/clients/{id}/update',function($p){
 ...
 try{
  ...
  if(in_array((string)($u['role']??''),['seller','supervisor'],true)&&!client_central_crm_accessible($id))throw new RuntimeException('Este cadastro não pertence à base CRM ativa.');
  ClientService::updateInOmie($id,$_POST,$u);
...
$router->get('/clients/{id}',function($p){
 ...
 if((string)($u['role']??'')!=='admin'&&(!empty($c['crm_inactive'])||(int)($c['active']??0)!==1)){http_response_code(404);exit('Cliente não encontrado.');}
 if(in_array((string)($u['role']??''),['seller','supervisor'],true)&&!client_central_crm_accessible($id)){http_response_code(404);exit('Cliente não encontrado.');}
...
$router->post('/clients/{id}/commercial-profile',function($p){
 ...
 if(!$client||(int)($client['active']??0)!==1||!empty($client['crm_inactive'])||(in_array((string)($u['role']??''),['seller','supervisor'],true)&&!client_central_crm_accessible($id))){http_response_code(404);exit('Cliente não encontrado.');}
...
$router->get('/api/clients/datatable',function(){
 ...
 if($sellerFilter==='__none__'){
  ...
  if($crmPortfolioReady&&str_starts_with($sellerFilter,'crm:')){$baseWhere[]='c.crm_owner_user_id=?';$baseParams[]=(int)substr($sellerFilter,4);}
  elseif(!$crmPortfolioReady){$baseWhere[]='('.$effectiveSellerSql.')=?';$baseParams[]=$sellerFilter;}
 }
 ...
 $rows=DB::all(
  "SELECT c.*,s.name seller_name,os.name omie_seller_name,(".$effectiveSellerExpr.") effective_seller_code,
          es.name effective_seller_name,cu.name crm_owner_name,cu.crm_user_omie_code crm_owner_code,
          COALESCE(cp.is_cfc,0) is_cfc,COALESCE(cp.is_reseller,0) is_reseller,cp.classification_source,
   ...
   LEFT JOIN client_commercial_profiles cp ON cp.client_id=c.id
   ".$sellerJoin."
 ...
 foreach($rows as $row){
  ...
  $rowTags=client_tags_from_raw($row['raw_json']??null);
  $tagsHtml='<div class="tdc-commercial-badges">';
  if(!empty($row['is_cfc']))$tagsHtml.='<span class="cfc"><i class="fa-solid fa-building-columns"></i>CFC</span>';
  if(!empty($row['is_reseller']))$tagsHtml.='<span class="reseller"><i class="fa-solid fa-handshake"></i>Revendedor</span>';
  if(empty($row['is_cfc'])&&empty($row['is_reseller']))$tagsHtml.='<small>Sem classificação</small>';
  if($rowTags)$tagsHtml.='<em title="'.e(implode(', ',$rowTags)).'">'.count($rowTags).' tag(s) Omie</em>';
  $tagsHtml.='</div>';
  ...
  if($canEdit){$editHref=APP_URL.'/clients/'.$id.'/edit';$actions.='<a class="client-action client-action-edit" href="'.e($editHref).'" title="Editar cadastro completo"><i class="fa-regular fa-pen-to-square"></i><span>Editar</span></a>';}
  if($canManage){
   if($crmInactive){
