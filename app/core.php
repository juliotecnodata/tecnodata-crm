...
final class DB {
 ...
 public static function sql(string $sql): string{
  $prefix=self::prefix();
  $tables=['commercial_partner_work_items','commercial_partner_work','commercial_sales','users','sellers','clients','client_tags','client_portfolio_assignments','client_seller_audit','client_metrics','crm_users','crm_accounts','crm_account_links','crm_account_link_audit','crm_account_commercial_profiles','crm_account_commercial_audit','crm_account_notes','crm_contacts','user_omie_identity','client_commercial_profiles','client_commercial_audit','sync_outbox','products','categories','departments','financial_accounts','order_stages','payment_terms','tax_scenarios','stock_locations','payment_methods','document_types','orders','local_order_drafts','service_orders','financial_movements','activities','tasks','collection_cases','collection_actions','settings','sync_state','omie_order_logs','goals','virtual_seller_goals','collection_assignment_log','order_profiles'];
...
final class Auth {
 ...
 public static function loginVerifiedEmail(string $email): bool{
  $u=DB::one("SELECT id,name,email,role,seller_omie_code,crm_user_omie_code,active FROM users WHERE email=? LIMIT 1",[mb_strtolower(trim($email))]);
