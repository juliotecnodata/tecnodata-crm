...
final class EnrollmentApiController
{
    public function upsert(): void
    {
        ...
        if ($code === '') {
            ApiAuth::deny(422, 'COURSE_REQUIRED', 'Código do curso é obrigatório.');
        }
        ...
        if ($idem !== '') {
            $prev = Database::fetch(
                "SELECT response_json FROM idempotency_keys
                  WHERE api_client_id=? AND idem_key=?",
                [$client['id'], $idem]
            );
            if ($prev) {
                ApiAuth::json(json_decode($prev['response_json'], true) ?: []);
            }
        }
        ...
        try {
            ...
            if (!$user) {
                ...
                if ($name === '') {
                    ApiAuth::deny(
                        404,
                        'STUDENT_NOT_FOUND',
                        'Aluno não encontrado. Envie student.name para criá-lo nesta mesma operação.'
                    );
                }
            ...
            $course = Database::fetch(
                "SELECT * FROM courses WHERE code=? OR shortname=? LIMIT 1",
                [$code, $code]
            );
            if (!$course) {
                $pdo->rollBack();
            ...
            if ($external !== '') {
                $byRef = Database::fetch(
                    "SELECT e.*
                       FROM enrollment_external_refs r
                       JOIN enrollments e ON e.id=r.enrollment_id
                      WHERE r.source_system=? AND r.external_id=?
                      LIMIT 1",
                    [$source, $external]
                );
                if ($byRef) {
            ...
            if (!$en) {
                Database::execute(
                    "INSERT INTO enrollments(
                        user_id,course_id,status,progress_percent,started_at,created_at,updated_at
                     ) VALUES(?,?,'active',0,?,?,?)",
                    [$user['id'],$course['id'],Clock::sql(),Clock::sql(),Clock::sql()]
                );
            ...
            if ($external !== '') {
                Database::execute(
                    "INSERT IGNORE INTO enrollment_external_refs(
                        enrollment_id,source_system,external_id,created_at
                     ) VALUES(?,?,?,?)",
    ...
    public function progress(string $id): void
    {
        ...
        if (!$e) {
            ApiAuth::deny(404,'ENROLLMENT_NOT_FOUND','Matrícula não encontrada.');
        }
        ...
        ApiAuth::json([
            ...
            'enrollment'=>[
                ...
                'started_at'=>Clock::iso($e['started_at']),
                'expires_at'=>Clock::iso($e['expires_at']),
    ...
    public function eligibility(): void
    {
        ...
        $e = Database::fetch(
            "SELECT e.*,c.code,c.name,u.name student_name,u.cpf
               ...
               JOIN users u ON u.id=e.user_id
               JOIN courses c ON c.id=e.course_id
              WHERE u.cpf=? AND (c.code=? OR c.shortname=?)
        ...
        $reasons = [];
        if ($e['status'] !== 'completed') $reasons[] = 'ENROLLMENT_NOT_COMPLETED';
        if ((float)$e['progress_percent'] < 100) $reasons[] = 'PROGRESS_INCOMPLETE';
    ...
    private function rememberIdempotency(int $clientId, string $idem, array $response): void
    {
        ...
        Database::execute(
            "INSERT IGNORE INTO idempotency_keys(api_client_id,idem_key,response_json,created_at)
             VALUES(?,?,?,?)",
            [
                $clientId,
                $idem,
