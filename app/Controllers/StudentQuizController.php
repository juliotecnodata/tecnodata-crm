...
final class StudentQuizController
{
    public function show(string $enrollmentId,string $activityId): void
    {
        ...
        $access=(new CourseRuleService())->canAccessActivity((int)$en['id'],(int)$activity['id']);
        if(!$access['allowed'])throw new \RuntimeException('Esta avaliação está bloqueada por sequência ou prazo.');

        $bio=(new BiometricPolicyService())->requirementForEnrollment((int)$en['id'],'before_quiz');

        $attempt=Database::fetch(
            "SELECT * FROM quiz_attempts WHERE quiz_id=? AND enrollment_id=? AND status='inprogress' ORDER BY id DESC LIMIT 1",
            [$quiz['id'],$en['id']]
        );
        if(!$attempt)$attempt=$this->createAttempt($quiz,$en);
    ...
    {
        $user=Auth::requireLogin();Csrf::verify();
        [$en,$activity,$quiz]=$this->context((int)$enrollmentId,(int)$activityId,(int)$user['id']);
        ...
        $ids=json_decode($attempt['question_order_json'],true)?:[];
        $questions=$this->loadQuestions($ids);
        $total=0.0;$earned=0.0;$responses=(array)($_POST['q']??[]);

        $pdo=Database::pdo();$pdo->beginTransaction();
        try{
            foreach($questions as $q){
                $mark=max(.01,(float)$q['default_mark']);$total+=$mark;
                $response=$responses[$q['id']]??null;
                $fraction=$this->gradeQuestion($q,$response);
                $questionMark=$mark*$fraction;$earned+=$questionMark;

                Database::execute(
                    "INSERT INTO quiz_attempt_answers(attempt_id,question_id,response_json,fraction,mark,answered_at)
                     VALUES(?,?,?,?,?,?)
                     ON DUPLICATE KEY UPDATE response_json=VALUES(response_json),fraction=VALUES(fraction),mark=VALUES(mark),answered_at=VALUES(answered_at)",
                    [$attempt['id'],$q['id'],json_encode($response,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$fraction,$questionMark,Clock::sql()]
                );
            }
            ...
            Database::execute(
                "UPDATE quiz_attempts SET status='finished',score=?,percent=?,passed=?,finished_at=? WHERE id=?",
            ...
            Database::execute("UPDATE enrollments SET final_score=?,updated_at=? WHERE id=?",[$best,Clock::sql(),$en['id']]);

            if($passed)(new ProgressService())->completeActivity((int)$en['id'],(int)$activity['id'],100);
    ...
    public function result(string $enrollmentId,string $activityId,string $attemptId): void
    {
        ...
        $attempt=Database::fetch(
            "SELECT * FROM quiz_attempts WHERE id=? AND quiz_id=? AND enrollment_id=?",
        ...
        $answers=Database::all(
            "SELECT qa.*,q.name,q.question_html,q.type
               FROM quiz_attempt_answers qa JOIN questions q ON q.id=qa.question_id
    ...
    private function createAttempt(array $quiz,array $en): array
    {
        ...
        if($allowed>0&&$count>=$allowed)throw new \RuntimeException('Limite de tentativas atingido.');
        ...
        Database::execute(
            "INSERT INTO quiz_attempts(quiz_id,enrollment_id,attempt_no,status,question_order_json,started_at,created_at)
             VALUES(?,?,?,'inprogress',?,?,?)",
            [$quiz['id'],$en['id'],$count+1,json_encode($ids),Clock::sql(),Clock::sql()]
        );
    ...
    {
        $slots=Database::all("SELECT * FROM quiz_slots WHERE quiz_id=? ORDER BY position,id",[$quizId]);
        $ids=[];
        foreach($slots as $slot){
            if($slot['question_id']){$ids[]=(int)$slot['question_id'];continue;}
            if($slot['category_id']&&(int)$slot['random_count']>0){
                $pool=array_map('intval',array_column(Database::all("SELECT id FROM questions WHERE category_id=? ORDER BY id",[$slot['category_id']]),'id'));
                shuffle($pool);$ids=array_merge($ids,array_slice($pool,0,(int)$slot['random_count']));
            }
        }
    ...
    private function loadQuestions(array $ids): array
    {
        ...
        foreach($ids as $id){
            ...
            if(!$q)continue;
            $q['answers']=Database::all("SELECT * FROM question_answers WHERE question_id=? ORDER BY position,id",[$q['id']]);
            $out[]=$q;
        }
        return $out;
    }
    ...
    private function gradeQuestion(array $q,mixed $response): float
    {
        ...
        if($q['type']==='multichoice'||$q['type']==='truefalse'){
            ...
            $fraction=0.0;
            foreach($q['answers'] as $a)if(in_array((string)$a['id'],array_map('strval',$selected),true))$fraction+=(float)$a['fraction'];
            return max(0,min(1,$fraction));
        }

        if($q['type']==='shortanswer'){
            $given=trim((string)$response);$case=!empty($settings['case_sensitive']);
        ...
        if($q['type']==='numerical'){
            ...
            foreach($q['answers'] as $a){
                ...
                if(is_numeric($expected)&&abs((float)$given-(float)$expected)<=$tol)return max(0,min(1,(float)$a['fraction']));
            }
            return 0;
        }
        ...
        if($q['type']==='matching'){
            ...
            $hits=0;
            foreach($pairs as $i=>$pair)if(trim((string)($response[$i]??''))===trim((string)($pair['right']??'')))$hits++;
    ...
    {
        $en=Database::fetch(
            "SELECT e.*,c.name course_name FROM enrollments e JOIN courses c ON c.id=e.course_id
              WHERE e.id=? AND e.user_id=? AND e.status IN('active','completed')",
            [$enrollmentId,$userId]
        );
        if(!$en)throw new \RuntimeException('Matrícula inválida.');

        $activity=Database::fetch("SELECT * FROM course_activities WHERE id=? AND course_id=? AND type='quiz'",[$activityId,$en['course_id']]);
