...
final class ClientService {
 ...
 private static function omieClientCoreUpdatePayload(array $remoteRow,string $code,string $inactive='S'): array{
  // O AlterarCliente recebe clientes_cadastro. Apesar de aceitar alterações parciais,
  // alguns cadastros não efetivam a troca de status quando enviamos somente chave+inativo.
  // Reenviamos os campos centrais do próprio cadastro remoto, sem tocar em endereço,
  // financeiro, tags ou demais dados operacionais.
  $payload=['codigo_cliente_omie'=>(int)$code];
  foreach(['codigo_cliente_integracao','razao_social','cnpj_cpf','nome_fantasia'] as $field){
   if(array_key_exists($field,$remoteRow)&&$remoteRow[$field]!==null&&$remoteRow[$field]!=='')$payload[$field]=$remoteRow[$field];
  }
 ...
 public static function deleteInactiveOmieBatchFromCrm(array $sourceIds,int $preferredTargetId,array $u,array $verifiedInactiveCodes=[],?array $preloadedAudit=null): array{
  ...
  foreach($sources as $source){
   ...
   elseif($document!==$digits)throw new RuntimeException('Todos os cadastros selecionados precisam pertencer ao mesmo CPF/CNPJ.');
  ...
  $locals=DB::all("SELECT id,omie_code,name,legal_name,active,crm_inactive
                   FROM clients
                   WHERE REGEXP_REPLACE(COALESCE(document,''),'[^0-9]','')=?
                   ORDER BY active DESC,crm_inactive ASC,id",[$document]);
  $sourceMap=array_fill_keys($sourceIds,true);$eligibleTargets=[];
  foreach($locals as $local){
   $id=(int)$local['id'];$code=trim((string)($local['omie_code']??''));
   if(isset($sourceMap[$id])||$code===''||str_starts_with($code,'LOCAL-')||empty($activeRemoteCodes[$code])||!empty($local['crm_inactive']))continue;
   $eligibleTargets[]=[
    'id'=>$id,'omie_code'=>$code,'name'=>(string)$local['name'],
    'legal_name'=>(string)($local['legal_name']??''),'active'=>(bool)$local['active'],
   ];
  }
  ...
  elseif(count($eligibleTargets)>1){
   return [
    ...
    'message'=>'Há mais de um cadastro ativo na Omie. Escolha qual deve permanecer no CRM e envie todos os inativos em uma única operação.',
   ];
  }
  $targetId=$target?(int)$target['id']:0;

  $historyBySource=[];$totalHistory=0;
  foreach($sources as $source){
   $id=(int)$source['id'];
   $counts=[
  ...
  try{
   ...
   foreach($sources as $source){
    ...
    if($targetId>0){
     ...
     try{DB::exec("UPDATE client_seller_audit SET client_id=? WHERE client_id=?",[$targetId,$sourceId]);}catch(Throwable $e){}
     try{DB::exec("INSERT IGNORE INTO client_tags(client_id,tag_key,tag) SELECT ?,tag_key,tag FROM client_tags WHERE client_id=?",[$targetId,$sourceId]);}catch(Throwable $e){}

     $sourceCase=DB::one("SELECT client_id FROM collection_cases WHERE client_id=?",[$sourceId]);
     if($sourceCase){
      $targetCase=DB::one("SELECT client_id FROM collection_cases WHERE client_id=?",[$targetId]);
      if($targetCase)DB::exec("DELETE FROM collection_cases WHERE client_id=?",[$sourceId]);
      else DB::exec("UPDATE collection_cases SET client_id=? WHERE client_id=?",[$targetId,$sourceId]);
     }
 ...
 public static function inactivateOmieAndDeleteFromCrm(int $id,int $preferredTargetId,array $u): array{
  ...
  // Uma consulta por CPF/CNPJ já traz todos os códigos do grupo.
  $audit=self::inspectOmieDocument($id);
  $remote=null;
  foreach((array)($audit['remote']??[]) as $candidate){
   if((string)($candidate['omie_code']??'')===$code){$remote=$candidate;break;}
  }

  // Se o código não existe mais na Omie, seguimos a regra já aprovada:
  // exclui somente do CRM e preserva/transfere o histórico.
  if(!$remote){
   $result=self::deleteInactiveOmieBatchFromCrm([$id],$preferredTargetId,$u,[],$audit);
   $result['remote_action']='not_found';
   $result['message']='O código Omie '.$code.' não foi encontrado. O cadastro foi removido somente do CRM; nenhuma alteração foi enviada à Omie.';
   return $result;
  }

  $alreadyInactive=!empty($remote['inactive']);
  if($alreadyInactive){
   $result=self::deleteInactiveOmieBatchFromCrm([$id],$preferredTargetId,$u,[$code],$audit);
   $result['remote_action']='already_inactive';
   $result['message']='O cadastro já estava inativo na Omie. '.($result['message']??'Cadastro removido somente do CRM.');
   return $result;
  }

  // Para inativar um cadastro ativo pela tela de duplicados, outro cadastro ativo deve permanecer.
  $activeTargets=[];
  foreach((array)($audit['remote']??[]) as $candidate){
   if(!empty($candidate['inactive']))continue;
   $candidateCode=trim((string)($candidate['omie_code']??''));
   $local=is_array($candidate['local']??null)?$candidate['local']:null;
   if($candidateCode===''||$candidateCode===$code||!$local||!empty($local['crm_inactive']))continue;
   $activeTargets[]=[
    'id'=>(int)$local['id'],'omie_code'=>$candidateCode,'name'=>(string)($local['name']??$candidate['name']??$candidateCode),
   ];
  }

  $target=null;
  if($preferredTargetId>0){
   foreach($activeTargets as $candidate)if((int)$candidate['id']===$preferredTargetId){$target=$candidate;break;}
   if(!$target)throw new RuntimeException('O cadastro escolhido para permanecer não está ativo na Omie ou não pertence ao mesmo CPF/CNPJ.');
  }elseif(count($activeTargets)===1)$target=$activeTargets[0];
  elseif(count($activeTargets)>1){
   return [
    'status'=>'target_required','requires_target'=>true,'audit'=>$audit,'candidates'=>$activeTargets,
    'source'=>['id'=>$id,'name'=>(string)$client['name'],'omie_code'=>$code],
    'message'=>'Existem vários cadastros ativos na Omie. Escolha “Manter este no CRM” no cadastro que deve permanecer e tente novamente.',
   ];
  }else{
   throw new RuntimeException('Este é o único cadastro ativo encontrado na Omie para este CPF/CNPJ. A inativação foi bloqueada para não deixar o documento sem cadastro ativo.');
  }
  ...
  $result['message']='Cadastro Omie '.$code.' inativado e removido somente do CRM. O cadastro “'.$target['name'].'” foi mantido como principal e recebeu o histórico local.';
  return $result;
 }
...
final class GoalService {
 ...
 private static function sellerProduction(string $sellerCode,string $start,string $next,array $days=[]): array{
  ...
  [$serviceDaysSql,$serviceDaysParams]=self::selectedDaysSql('service_date',$days);
  $services=(float)(DB::scalar(
   "SELECT COALESCE(SUM(total),0) FROM service_orders
    WHERE seller_omie_code=? AND service_date>=? AND service_date<?
      AND UPPER(COALESCE(status,'')) NOT LIKE '%CANCEL%'".$serviceDaysSql,
   array_merge([$sellerCode,$start,$next],$serviceDaysParams)
  )??0);

  return [
   'orders'=>$orders,
   'orders_without_freight'=>$ordersWithoutFreight,
   'services'=>$services,
   'commercial_without_freight'=>$ordersWithoutFreight+$services,
 ...
 public static function userMonth(int $userId,string $month,array $days=[]): array{
  ...
  return [
   'user'=>$u,'goal'=>$g,'month'=>$month,'days'=>$days,'goal_scope'=>$days?'Meta proporcional aos dias':'Meta mensal','sales'=>$sales,'orders_sales'=>$ordersSales,'orders_without_freight'=>$ordersWithoutFreight,'services_sales'=>$servicesSales,'commercial_without_freight'=>$ordersWithoutFreight+$servicesSales,
 ...
 public static function managementMonth(string $month,array $days=[]): array{
  ...
  foreach(DB::all(
   "SELECT seller_omie_code,COALESCE(SUM(total),0) total
    ...
    WHERE service_date>=? AND service_date<?
      ...
      AND seller_omie_code IS NOT NULL AND seller_omie_code<>''".$serviceDaysSql."
    GROUP BY seller_omie_code",
   array_merge([$start,$next],$serviceDaysParams)
  ) as $r){
   $code=(string)$r['seller_omie_code'];
   $servicesMap[$code]=(float)$r['total'];
  }
  ...
  foreach($users as $u){
   ...
   $row=[
    'user'=>$u,'goal'=>$g,'sales'=>$sales,'orders_sales'=>$ordersSales,'orders_without_freight'=>$ordersWithoutFreight,'services_sales'=>$servicesSales,'commercial_without_freight'=>$ordersWithoutFreight+$servicesSales,
  ...
  foreach($allSellers as $seller){
   ...
   $tot=$o+$sv;
   $sales+=$tot;$orderSales+=$o;$orderSalesWithoutFreight+=$oWithoutFreight;$serviceSales+=$sv;
   if(self::isVirtualSellerName((string)$seller['name'])){
    ...
    $virtualRows[]=[
     ...
     'services'=>$sv,
     'commercial_without_freight'=>$oWithoutFreight+$sv,
     'sales'=>$tot,
  ...
  return [
   ...
   'general_goal'=>$general,'sales'=>$sales,'order_sales'=>$orderSales,'order_sales_without_freight'=>$orderSalesWithoutFreight,'service_sales'=>$serviceSales,'commercial_without_freight'=>$orderSalesWithoutFreight+$serviceSales,
