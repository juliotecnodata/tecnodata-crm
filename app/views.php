<?php
function render_commercial_activity_dialog(array $vars): void{
 $u=$vars['user']??Auth::user();$types=$vars['types']??[];$channels=$vars['channels']??[];$categories=$vars['categories']??[];$assignable=$vars['assignable']??[];
 $returnTo=(string)($vars['return_to']??'account');$returnQuery=(string)($vars['return_query']??'');
 ?>
 <dialog class="tda-dialog" data-commercial-activity-dialog aria-labelledby="tda-dialog-title">
  <form method="post" data-commercial-activity-form>
   ...
   <div class="tda-client">
    <span class="tda-client-icon"><i class="fa-solid fa-briefcase"></i></span>
    <div class="tda-client-copy"><span>Cliente: <strong data-commercial-activity-client>Conta CRM</strong></span><small>Responsável: <b data-commercial-activity-owner>—</b></small></div>
    <span class="tda-client-type" data-commercial-activity-type>Cliente</span>
   </div>
   <div class="tda-body">
    <fieldset class="tda-section tda-activity-types">
     <legend>Tipo de atividade</legend>
     <div class="tda-type-grid">
      <?php foreach($types as $idx=>$type):?><label class="tda-type-card" data-activity-card="<?=e((string)$type['code'])?>"><input type="radio" name="activity_type" value="<?=e((string)$type['code'])?>" <?=$idx===0?'checked':''?>><span><i class="fa-solid <?=e((string)$type['icon'])?>"></i><b><?=e((string)$type['label'])?></b><em><i class="fa-solid fa-circle-check"></i></em></span></label><?php endforeach;?>
     </div>
    </fieldset>

    <fieldset class="tda-section">
     <legend>Canal do contato</legend>
     <div class="tda-chip-row tda-channel-row">
      <?php foreach($channels as $idx=>$channel):$icon=(string)$channel['icon'];$iconClass=str_starts_with($icon,'fa-brands ')?$icon:'fa-solid '.$icon;?><label class="tda-chip"><input type="radio" name="channel" value="<?=e((string)$channel['code'])?>" <?=$idx===0?'checked':''?> required><span><i class="<?=e($iconClass)?>"></i><?=e((string)$channel['label'])?><b><i class="fa-solid fa-circle-check"></i></b></span></label><?php endforeach;?>
     </div>
    </fieldset>
    ...
    <div class="tda-section tda-notes">
     <label for="tda-notes">O que foi tratado</label>
   ...
   <footer class="tda-footer"><button type="button" class="tda-btn secondary" data-commercial-activity-close>Cancelar</button><button type="submit" class="tda-btn primary"><i class="fa-regular fa-floppy-disk"></i>Salvar atividade</button></footer>
  </form>
 </dialog>
 <?php
}
...
function render(string $name,array $vars=[]): void{
 ...
 switch($name){
   ...
   <section class="tdh-home">
    ...
    <section class="tdh-portfolio">
     ...
     <div class="tdh-table-wrap">
      <table class="tdh-table" data-datatable data-page-length="10">
       ...
       ?>
        <tr>
         ...
         <td class="tdh-col-actions"><div class="tdh-actions">
          <button type="button" class="attempt" title="Registrar tentativa de contato" aria-label="Registrar tentativa de contato" data-commercial-activity-open data-activity-type="contact_attempt" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($typeLabel)?>" data-client-id="<?=!empty($row['client_id'])?(int)$row['client_id']:''?>"><i class="fa-solid fa-phone"></i><span class="tdh-action-label">Tentativa</span></button>
          <button type="button" class="contact" title="Registrar contato" aria-label="Registrar contato" data-commercial-activity-open data-activity-type="contact_completed" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($typeLabel)?>" data-client-id="<?=!empty($row['client_id'])?(int)$row['client_id']:''?>"><i class="fa-solid fa-comment-dots"></i><span class="tdh-action-label">Contato</span></button>
          <button type="button" class="follow" title="Registrar follow-up" aria-label="Registrar follow-up" data-commercial-activity-open data-activity-type="follow_up" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($typeLabel)?>" data-client-id="<?=!empty($row['client_id'])?(int)$row['client_id']:''?>"><i class="fa-solid fa-clipboard-check"></i><span class="tdh-action-label">Follow-up</span></button>
          <?php if($linked):?><button type="button" class="sale" title="Registrar venda" aria-label="Registrar venda" data-commercial-activity-open data-activity-type="sale" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($typeLabel)?>" data-client-id="<?=!empty($row['client_id'])?(int)$row['client_id']:''?>"><i class="fa-solid fa-circle-dollar-to-slot"></i><span class="tdh-action-label">Venda</span></button><?php else:?><span class="sale disabled" title="A venda exige Cliente Geral vinculado" aria-disabled="true"><i class="fa-solid fa-circle-dollar-to-slot"></i><span class="tdh-action-label">Venda</span></span><?php endif;?>
         </div></td>
        </tr>
       <?php endforeach;?>
    ...
    <?php render_commercial_activity_dialog([
     ...
     'return_to'=>'home','return_query'=>http_build_query(array_merge($homeFilters,['home_page'=>$homePage]))
    ]);?>
   </section>
  <?php break;
  case 'dashboard':
     ...
     <section class="tdres" id="resultados">
       ...
       <?php else:?>
        ...
        <div class="tdres-bottom-grid">
         <section class="tdres-panel">
          ...
          <div class="tdres-table-wrap"><table class="tdres-table" data-datatable data-page-length="10"><thead><tr><th>Cobrador</th><th>Meta</th><th>Recuperado</th><th>Falta</th><th>Ações</th><th>Atingimento</th><th>Status</th></tr></thead><tbody>
          <?php foreach($teamRows as $row):$status=$statusFor($row['percent']);?>
      ...
      <?php elseif($model==='executive'):?>
       <section class="tdres-panel">
        ...
        <div class="tdres-table-wrap"><table class="tdres-table" data-datatable data-page-length="10"><thead><tr><th>#</th><th>Vendedor</th><th>Meta</th><th>Realizado</th><th>Pedidos</th><th>Serviços</th><th>Contatos</th><th>Atingimento</th><th>Status</th></tr></thead><tbody>
  ...
  case 'client_audit':
   ...
   $auditUf=$auditUf??'';$auditSeller=$auditSeller??'';$auditTag=$auditTag??'';$auditSource=$auditSource??'all';$auditMinRecords=$auditMinRecords??2;$auditDateFrom=$auditDateFrom??'';$auditDateTo=$auditDateTo??'';$auditSort=$auditSort??'records_desc';$auditUfs=$auditUfs??[];$auditSellers=$auditSellers??[];$auditTags=$auditTags??[];
   $auditUrl=static function(array $changes=[])use($auditTab,$auditQuery,$auditConflict,$auditMonth,$auditUf,$auditSeller,$auditTag,$auditSource,$auditMinRecords,$auditDateFrom,$auditDateTo,$auditSort){$params=['tab'=>$auditTab,'q'=>$auditQuery,'conflict'=>$auditConflict,'month'=>$auditMonth,'uf'=>$auditUf,'seller'=>$auditSeller,'tag'=>$auditTag,'source'=>$auditSource,'min_records'=>$auditMinRecords,'date_from'=>$auditDateFrom,'date_to'=>$auditDateTo,'sort'=>$auditSort];foreach($changes as $key=>$value){if($value===null||$value===''||$value==='all'||($key==='min_records'&&(int)$value===2)||($key==='sort'&&$value==='records_desc'))unset($params[$key]);else $params[$key]=$value;}return APP_URL.'/clients-audit?'.http_build_query($params);};
   ?>
   <section class="tdaudit-page">
    ...
    <form class="tdaudit-filter <?=$auditTab==='duplicates'?'duplicates':''?>" method="get" action="<?=APP_URL?>/clients-audit">
     ...
     <label class="tdaudit-search"><span>Buscar cadastro</span><div><i class="fa-solid fa-magnifying-glass"></i><input type="search" name="q" value="<?=e($auditQuery)?>" placeholder="Nome, CPF/CNPJ, cidade ou código Omie"></div></label>
     <?php if($auditTab==='duplicates'):?>
      <label><span>Tipo de conflito</span><select name="conflict" class="form-select"><option value="all">Todos</option><option value="active" <?=$auditConflict==='active'?'selected':''?>>Múltiplos ativos</option><option value="mixed" <?=$auditConflict==='mixed'?'selected':''?>>Ativo + inativo</option><option value="inactive" <?=$auditConflict==='inactive'?'selected':''?>>Somente inativos</option><option value="invalid" <?=$auditConflict==='invalid'?'selected':''?>>Documento inválido</option></select></label>
      <label><span>UF</span><select name="uf" class="form-select"><option value="">Todas</option><?php foreach($auditUfs as $option):$uf=(string)($option['uf']??'');?><option value="<?=e($uf)?>" <?=$auditUf===$uf?'selected':''?>><?=e($uf)?></option><?php endforeach;?></select></label>
      <label><span>Vendedor</span><select name="seller" class="form-select"><option value="">Todos</option><?php foreach($auditSellers as $option):$code=(string)($option['omie_code']??'');?><option value="<?=e($code)?>" <?=$auditSeller===$code?'selected':''?>><?=e((string)($option['name']??$code))?></option><?php endforeach;?></select></label>
      <label><span>Tag</span><select name="tag" class="form-select"><option value="">Todas as tags</option><?php foreach($auditTags as $option):$tag=(string)($option['tag']??'');if($tag==='')continue;?><option value="<?=e($tag)?>" <?=mb_strtolower($auditTag,'UTF-8')===mb_strtolower($tag,'UTF-8')?'selected':''?>><?=e($tag)?><?php if(isset($option['total'])):?> (<?=number_format((int)$option['total'],0,',','.')?>)<?php endif;?></option><?php endforeach;?></select></label>
      <label><span>Origem</span><select name="source" class="form-select"><option value="all">Todos</option><option value="omie_only" <?=$auditSource==='omie_only'?'selected':''?>>Somente Omie</option><option value="with_local" <?=$auditSource==='with_local'?'selected':''?>>Com cadastro local</option></select></label>
      <label><span>Mínimo de registros</span><select name="min_records" class="form-select"><?php foreach([2,3,4,5,10] as $n):?><option value="<?=$n?>" <?=(int)$auditMinRecords===$n?'selected':''?>><?=$n?> ou mais</option><?php endforeach;?></select></label>
      <label><span>Inclusão de</span><input class="form-control" type="date" name="date_from" value="<?=e($auditDateFrom)?>"></label>
      <label><span>Inclusão até</span><input class="form-control" type="date" name="date_to" value="<?=e($auditDateTo)?>"></label>
      <label><span>Ordenar por</span><select name="sort" class="form-select"><option value="records_desc" <?=$auditSort==='records_desc'?'selected':''?>>Mais duplicados</option><option value="oldest" <?=$auditSort==='oldest'?'selected':''?>>Inclusão mais antiga</option><option value="newest" <?=$auditSort==='newest'?'selected':''?>>Inclusão mais recente</option><option value="updated_oldest" <?=$auditSort==='updated_oldest'?'selected':''?>>Atualização mais antiga</option><option value="updated_newest" <?=$auditSort==='updated_newest'?'selected':''?>>Atualização mais recente</option></select></label>
     <?php elseif($auditTab==='responsibility'):?><label><span>Mês da carteira</span><input class="form-control" type="month" name="month" value="<?=e($auditMonth)?>"></label><?php else:?><input type="hidden" name="month" value="<?=e($auditMonth)?>"><?php endif;?>
     <div class="tdaudit-filter-actions"><button class="tdaudit-btn primary" type="submit"><i class="fa-solid fa-filter"></i>Aplicar filtros</button>
     <?php $hasAuditFilter=$auditQuery!==''||$auditConflict!=='all'||$auditUf!==''||$auditSeller!==''||$auditTag!==''||$auditSource!=='all'||(int)$auditMinRecords!==2||$auditDateFrom!==''||$auditDateTo!==''||$auditSort!=='records_desc';if($hasAuditFilter):?><a class="tdaudit-btn" href="<?=APP_URL?>/clients-audit?<?=e(http_build_query(['tab'=>$auditTab,'month'=>$auditMonth]))?>"><i class="fa-solid fa-xmark"></i>Limpar</a><?php endif;?></div>
    </form>

    <?php if($auditTab==='duplicates'):?>
     <div class="tdaudit-section-head"><div><span>DUPLICIDADES ENCONTRADAS</span><h2>Compare os registros do mesmo documento</h2><p>Exibindo <?=$auditPagination['from']?>–<?=$auditPagination['to']?> de <?=number_format((int)$auditPagination['total'],0,',','.')?> grupos filtrados.<?php if($auditTag!==''):?> Tag: <strong><?=e($auditTag)?></strong>.<?php endif;?></p></div><div class="tdaudit-legend"><span><i class="active"></i>Ativo</span><span><i class="inactive"></i>Inativo</span><span><i class="local"></i>Cadastro local</span></div></div>
     <div class="tdaudit-groups">
      <?php foreach($auditGroups as $group):$groupRows=$auditRowsByDocument[(string)$group['document_digits']]??[];$conflictLabel=$group['conflict_type']==='active'?'Múltiplos ativos':($group['conflict_type']==='mixed'?'Ativo + inativo':'Múltiplos inativos');?>
       <article class="tdaudit-group">
        ...
        <div class="tdaudit-table-wrap"><table class="tdaudit-table" data-datatable data-server-paged="1" data-page-length="10"><thead><tr><th>Status CRM</th><th>Cliente</th><th>Código Omie</th><th>Vendedor</th><th>Localização</th><th>Histórico</th><th>Inclusão</th><th>Atualizado</th><th>Ação</th></tr></thead><tbody>
    ...
    <?php elseif($auditTab==='responsibility'):?>
     ...
     <div class="tdaudit-inactive-card"><div class="tdaudit-table-wrap"><table class="tdaudit-table responsibility-list" data-datatable data-server-paged="1" data-page-length="10"><thead><tr><th>Cliente</th><th>Principal CRM</th><th>Carteira <?=$auditMonthLabel?></th><th>Omie</th><th>Situação</th><th>Atualizado</th><th>Ação</th></tr></thead><tbody>
    ...
    <?php else:?>
     ...
     <div class="tdaudit-inactive-card"><div class="tdaudit-table-wrap"><table class="tdaudit-table inactive-list" data-datatable data-server-paged="1" data-page-length="10"><thead><tr><th>Cliente</th><th>CPF/CNPJ</th><th>Código Omie</th><th>Vendedor</th><th>Histórico encontrado</th><th>Cadastro ativo correspondente</th><th>Atualizado</th><th>Ação</th></tr></thead><tbody>
  ...
  case 'commercial_portfolio':
   ...
   <section class="tdp-portfolio">
    ...
    <section class="tdp-table-card">
     <?php
      $portfolioDataParams=['view'=>'portfolio','classification'=>(string)($filters['classification']??'all'),'attention'=>(string)($filters['attention']??'all')];
      if(!empty($filters['q']))$portfolioDataParams['q']=(string)$filters['q'];
     ?>
     <div class="tdp-table-wrap">
      <table class="tdp-table" data-datatable data-server-url="<?=APP_URL?>/api/commercial/accounts/datatable?<?=e(http_build_query($portfolioDataParams))?>" data-page-length="10" data-length-change="1">
       ...
       <?php foreach($rows as $row):
        ...
        $phone=trim((string)($row['contact_phone']??''));if($phone==='')$phone=trim((string)($row['contact_mobile']??''));if($phone==='')$phone=trim((string)($row['client_phone']??''));
       ...
       ?>
        <tr>
         ...
         <td><div class="tdh-actions tdp-home-actions">
          <?php if($canWork):?>
           <button type="button" class="attempt" title="Registrar tentativa de contato" aria-label="Registrar tentativa de contato" data-commercial-activity-open data-activity-type="contact_attempt" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($classification)?>" data-client-id="<?=$linked?(int)$row['client_id']:''?>"><i class="fa-solid fa-phone"></i><span class="tdh-action-label">Tentativa</span></button>
           <button type="button" class="contact" title="Registrar contato" aria-label="Registrar contato" data-commercial-activity-open data-activity-type="contact_completed" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($classification)?>" data-client-id="<?=$linked?(int)$row['client_id']:''?>"><i class="fa-solid fa-comment-dots"></i><span class="tdh-action-label">Contato</span></button>
           <button type="button" class="follow" title="Registrar follow-up" aria-label="Registrar follow-up" data-commercial-activity-open data-activity-type="follow_up" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($classification)?>" data-client-id="<?=$linked?(int)$row['client_id']:''?>"><i class="fa-solid fa-clipboard-check"></i><span class="tdh-action-label">Follow-up</span></button>
           <?php if($linked):?><button type="button" class="sale" title="Registrar venda" aria-label="Registrar venda" data-commercial-activity-open data-activity-type="sale" data-account-code="<?=e((string)$row['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($row['owner_name']?:$u['name']))?>" data-account-type="<?=e($classification)?>" data-client-id="<?=(int)$row['client_id']?>"><i class="fa-solid fa-circle-dollar-to-slot"></i><span class="tdh-action-label">Venda</span></button><?php else:?><span class="sale disabled" title="A venda exige Cliente Geral vinculado" aria-disabled="true"><i class="fa-solid fa-circle-dollar-to-slot"></i><span class="tdh-action-label">Venda</span></span><?php endif;?>
          <?php else:?>
           <span class="attempt disabled" title="Ações comerciais restritas à carteira responsável"><i class="fa-solid fa-phone"></i><span class="tdh-action-label">Tentativa</span></span>
           <span class="contact disabled" title="Ações comerciais restritas à carteira responsável"><i class="fa-solid fa-comment-dots"></i><span class="tdh-action-label">Contato</span></span>
           <span class="follow disabled" title="Ações comerciais restritas à carteira responsável"><i class="fa-solid fa-clipboard-check"></i><span class="tdh-action-label">Follow-up</span></span>
           <span class="sale disabled" title="Ações comerciais restritas à carteira responsável"><i class="fa-solid fa-circle-dollar-to-slot"></i><span class="tdh-action-label">Venda</span></span>
          <?php endif;?>
         </div></td>
        </tr>
       <?php endforeach;?>
   ...
   <section class="tdw-page tdw-portfolio">
    ...
    <section class="tdw-workqueue">
     ...
     <div class="tdw-account-list">
      <?php foreach($rows as $row):
       $display=trim((string)($row['trade_name']??''))?:trim((string)($row['name']??''));$linked=!empty($row['client_id']);$rowTypeLabel=!empty($row['is_cfc'])&&!empty($row['is_reseller'])?'CFC + Revendedor':(!empty($row['is_cfc'])?'CFC':(!empty($row['is_reseller'])?'Revendedor':($linked?'Cliente vinculado':'Prospect')));
       $days=(int)($row['days_without_contact']??999999);$nextAt=!empty($row['next_due_at'])?strtotime((string)$row['next_due_at']):false;$today=date('Y-m-d');
       $priority='normal';$priorityLabel='Sem retorno';
       if($nextAt&&date('Y-m-d',$nextAt)<$today){$priority='danger';$priorityLabel='Atrasado';}
       elseif($nextAt&&date('Y-m-d',$nextAt)===$today){$priority='today';$priorityLabel='Hoje';}
  ...
  case 'commercial_account':
   ...
   $classificationLabel=$isCfc&&$isReseller?'CFC + Revendedor':($isCfc?'CFC':($isReseller?'Revendedor':'Sem classificação'));
   $avgInterval=(float)($account['avg_interval_days']??0);
   if(empty($account['last_purchase_at']))$purchaseCycle='Sem histórico';
   elseif($avgInterval<=0)$purchaseCycle='Esporádico';
   elseif($avgInterval<=45)$purchaseCycle='Mensal';
   elseif($avgInterval<=120)$purchaseCycle='Trimestral';
   elseif($avgInterval<=220)$purchaseCycle='Semestral';
   elseif($avgInterval<=400)$purchaseCycle='Anual';
   else $purchaseCycle='Esporádico';

   $lastContactAt=null;
   foreach($activities as $activity){
    if(in_array((string)($activity['activity_type']??''),['contact_completed','follow_up'],true)){$lastContactAt=(string)$activity['created_at'];break;}
   }
   $daysWithoutContact=null;
   if($lastContactAt){$contactDay=strtotime(date('Y-m-d',strtotime($lastContactAt)));$todayDay=strtotime(date('Y-m-d'));$daysWithoutContact=max(0,(int)(($todayDay-$contactDay)/86400));}

   $returnRows=[];$returnSeen=[];$nextReturn=null;
   foreach($activities as $activity){
    $taskId=(int)($activity['return_task_id']??0);$due=(string)($activity['return_due_at']??'');if($taskId<=0||$due===''||isset($returnSeen[$taskId]))continue;
    $returnSeen[$taskId]=true;
    $row=['id'=>$taskId,'status'=>(string)($activity['return_task_status']??'pending'),'due_at'=>$due,'activity'=>$activity];
    $returnRows[]=$row;
    if($row['status']==='pending'&&(!$nextReturn||strtotime($row['due_at'])<strtotime((string)$nextReturn['due_at'])))$nextReturn=$row;
   }
   usort($returnRows,static function($a,$b){$ap=$a['status']==='pending'?0:1;$bp=$b['status']==='pending'?0:1;if($ap!==$bp)return $ap<=>$bp;return strtotime((string)$b['due_at'])<=>strtotime((string)$a['due_at']);});
   $returnCount=count($returnRows);
   $saleActivities=array_values(array_filter($activities,static fn($row)=>(string)($row['activity_type']??'')==='sale'));$saleCount=count($saleActivities);

   $nextDueTs=$nextReturn?strtotime((string)$nextReturn['due_at']):false;
   $nextLate=$nextDueTs!==false&&$nextDueTs<time();
   if($nextReturn)$liveStatus=$nextLate?'Retorno atrasado':'Retorno agendado';
   elseif($daysWithoutContact===null||$daysWithoutContact>30)$liveStatus='Precisa contato';
   else $liveStatus='Em dia';

   $classificationFromRaw=static function($raw): string{
    $data=is_string($raw)&&$raw!==''?json_decode($raw,true):null;if(!is_array($data))return 'Sem classificação';
    $c=!empty($data['cfc']);$r=!empty($data['reseller']);
    return $c&&$r?'CFC + Revendedor':($c?'CFC':($r?'Revendedor':'Sem classificação'));
   };
   $classificationAudit=array_values(array_filter($audit,static fn($change)=>(string)($change['field_name']??'')==='classification'));

   $channelView=static function(string $channel): array{
    return match($channel){
     'whatsapp'=>['WhatsApp','fa-brands fa-whatsapp','whatsapp'],
     'phone'=>['Ligação','fa-solid fa-phone','phone'],
     'email'=>['E-mail','fa-solid fa-envelope','email'],
     'presential'=>['Presencial','fa-solid fa-user-group','presential'],
     default=>[CommercialActivityService::channelLabel($channel),'fa-solid fa-message','other']
    };
   };
   $renderActivityRows=static function(array $rows)use($channelView): void{
    foreach($rows as $activity):
     $type=(string)($activity['activity_type']??'contact_completed');$category=(string)($activity['category_code']??'');$channel=(string)($activity['channel']??'other');
     [$channelLabel,$channelIcon,$channelTone]=$channelView($channel);
     if($type==='sale'){$channelLabel='Venda';$channelIcon='fa-solid fa-cart-shopping';$channelTone='sale';}
     $title=$category!==''?CommercialActivityService::categoryLabel($category):CommercialActivityService::activityTypeLabel($type);
     $createdTs=strtotime((string)$activity['created_at']);
     ?>
      <article class="tdf-activity <?=e($type)?>">
       <span class="tdf-dot"></span>
       <time><b><?=date('d/m',$createdTs)?></b><small><?=date('H:i',$createdTs)?></small></time>
       <span class="tdf-channel <?=e($channelTone)?>"><i class="<?=e($channelIcon)?>"></i><?=e($channelLabel)?></span>
       <div class="tdf-activity-card">
        <strong><?=e($title)?></strong>
        <p><?=!empty($activity['notes'])?nl2br(e((string)$activity['notes'])):'Atividade registrada sem descrição adicional.'?></p>
        <small>Por <?=e((string)($activity['user_name']??'Usuário'))?></small>
       </div>
      </article>
     <?php endforeach;
   };
   ...
   <section class="tdf-page">
    ...
    <header class="tdf-header">
     <div class="tdf-customer">
      <span class="tdf-customer-icon"><i class="fa-regular fa-building"></i></span>
      <div class="tdf-customer-main">
       <div class="tdf-title">
        <h1><?=e($display!==''?$display:'Conta sem nome')?></h1>
       ...
       <div class="tdf-customer-meta">
        <span><i class="fa-regular fa-user"></i>Responsável: <b><?=e((string)($account['owner_name']?:'Sem responsável'))?></b></span>
        <span><i class="fa-regular fa-calendar"></i>Período de compra: <b><?=e($purchaseCycle)?></b></span>
       </div>
      </div>
     </div>

     <div class="tdf-header-actions">
      <div class="tdf-statuses">
       <span class="status <?=e($liveStatus==='Retorno atrasado'?'late':($liveStatus==='Em dia'?'ok':'attention'))?>"><i class="fa-solid <?=e($liveStatus==='Retorno atrasado'?'fa-circle-exclamation':($liveStatus==='Em dia'?'fa-circle-check':'fa-bell'))?>"></i><?=e($liveStatus)?></span>
       <span class="days"><i class="fa-regular fa-calendar-check"></i><?=$daysWithoutContact===null?'Sem contato registrado':$daysWithoutContact.' dia'.($daysWithoutContact===1?'':'s').' desde o último contato'?></span>
      </div>
      <?php if($canWork):?><button class="tdf-new" type="button" data-commercial-activity-open data-activity-type="contact_completed" data-account-code="<?=e((string)$account['omie_code'])?>" data-account-name="<?=e($display!==''?$display:'Conta CRM')?>" data-account-owner="<?=e((string)($account['owner_name']?:$u['name']))?>" data-account-type="<?=e($classificationLabel)?>" data-client-id="<?=$linked?(int)$account['client_id']:''?>"><i class="fa-solid fa-plus"></i>Nova atividade</button><?php endif;?>
     </div>
    </header>
    ...
    <nav class="tdf-tabs" aria-label="Ficha do cliente">
     ...
     <button type="button" data-tdf-tab="activities"><i class="fa-regular fa-file-lines"></i>Atividades <b><?=$activityCount?></b></button>
     <button type="button" data-tdf-tab="returns"><i class="fa-regular fa-clock"></i>Retornos <b><?=$returnCount?></b></button>
     <button type="button" data-tdf-tab="sales"><i class="fa-solid fa-chart-column"></i>Vendas <b><?=$saleCount?></b></button>
     <button type="button" data-tdf-tab="observations"><i class="fa-regular fa-note-sticky"></i>Observações <b><?=$noteCount?></b></button>
    </nav>

    <div class="tdf-tab-panel active" data-tdf-panel="overview">
     <div class="tdf-overview">
      <section class="tdf-card tdf-history">
       <header><div><strong>Histórico de atividades</strong><small>Todas as interações e atividades realizadas com este cliente.</small></div></header>
       <div class="tdf-timeline">
        <?php $renderActivityRows(array_slice($activities,0,8));?>
        <?php if(!$activities):?><div class="tdf-empty"><i class="fa-regular fa-comments"></i><strong>Nenhuma atividade registrada</strong><span>Use “Nova atividade” para iniciar o histórico deste cliente.</span></div><?php endif;?>
       </div>
       <?php if($activityCount>8):?><footer><button type="button" data-tdf-open-tab="activities">Ver histórico completo</button></footer><?php endif;?>
      </section>

      <aside class="tdf-side">
       <section class="tdf-card tdf-next <?=$nextLate?'late':''?>">
        <header><span><i class="fa-regular fa-calendar"></i></span><strong>Próxima ação</strong></header>
        <?php if($nextReturn&&$nextDueTs!==false):$nextActivity=$nextReturn['activity'];?>
         <h3><?=$nextLate?'Retorno atrasado':'Retorno agendado'?> <?=date('d/m',$nextDueTs)?> às <?=date('H:i',$nextDueTs)?></h3>
         <p><?=!empty($nextActivity['notes'])?e((string)$nextActivity['notes']):'Retomar o contato conforme combinado na atividade anterior.'?></p>
         <small>Gerado pela atividade de <?=date('d/m/Y',strtotime((string)$nextActivity['created_at']))?></small>
         <?php if($canWork):?><div class="tdf-next-actions">
          <form method="post" action="<?=APP_URL?>/commercial/accounts/<?=rawurlencode((string)$account['omie_code'])?>/returns/<?=(int)$nextReturn['id']?>/complete"><input type="hidden" name="_token" value="<?=CSRF::token()?>"><button type="submit" class="primary"><i class="fa-solid fa-check"></i>Concluir retorno</button></form>
          <button type="button" class="secondary" data-commercial-activity-open data-activity-type="follow_up" data-account-code="<?=e((string)$account['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($account['owner_name']?:$u['name']))?>" data-account-type="<?=e($classificationLabel)?>" data-client-id="<?=$linked?(int)$account['client_id']:''?>">Registrar atividade</button>
         </div><?php endif;?>
        <?php else:?>
         <h3>Nenhum retorno agendado</h3>
         <p>Não existe próximo passo pendente para esta conta.</p>
         <?php if($canWork):?><button type="button" class="tdf-next-single" data-commercial-activity-open data-activity-type="follow_up" data-account-code="<?=e((string)$account['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($account['owner_name']?:$u['name']))?>" data-account-type="<?=e($classificationLabel)?>" data-client-id="<?=$linked?(int)$account['client_id']:''?>">Registrar atividade</button><?php endif;?>
        <?php endif;?>
       </section>

       <section class="tdf-card tdf-observation-summary">
        <header><span><i class="fa-regular fa-clipboard"></i></span><strong>Observações</strong><button type="button" data-tdf-open-tab="observations" title="Abrir observações"><i class="fa-solid fa-pen"></i></button></header>
        <?php if(isset($commercialNotes[0])):?><p><?=nl2br(e((string)$commercialNotes[0]['note']))?></p><small><?=e((string)$commercialNotes[0]['user_name'])?> · <?=date('d/m/Y H:i',strtotime((string)$commercialNotes[0]['created_at']))?></small><?php else:?><p class="muted">Nenhuma observação livre registrada para este cliente.</p><?php endif;?>
        <footer>Observações livres ficam disponíveis para consulta e não contam como atividade.</footer>
       </section>
      </aside>
     </div>
    </div>

    <div class="tdf-tab-panel" data-tdf-panel="activities">
     <section class="tdf-card tdf-full">
      <header><div><strong>Atividades</strong><small>Histórico completo do relacionamento, do registro mais recente ao mais antigo.</small></div><span><?=$activityCount?> registro<?=$activityCount===1?'':'s'?></span></header>
      <div class="tdf-timeline"><?php $renderActivityRows($activities);?><?php if(!$activities):?><div class="tdf-empty"><i class="fa-regular fa-comments"></i><strong>Nenhuma atividade registrada</strong></div><?php endif;?></div>
     </section>
    </div>

    <div class="tdf-tab-panel" data-tdf-panel="returns">
     <section class="tdf-card tdf-full">
      <header><div><strong>Retornos</strong><small>Retornos gerados a partir das atividades do cliente.</small></div><span><?=$returnCount?> retorno<?=$returnCount===1?'':'s'?></span></header>
      <div class="tdf-return-list">
       <?php foreach($returnRows as $ret):$retTs=strtotime((string)$ret['due_at']);$retLate=$ret['status']==='pending'&&$retTs<time();?>
        <article>
         <time><b><?=date('d/m',$retTs)?></b><small><?=date('H:i',$retTs)?></small></time>
         <div><strong><?=e(CommercialActivityService::activityTypeLabel((string)($ret['activity']['activity_type']??'follow_up')))?></strong><p><?=!empty($ret['activity']['notes'])?e((string)$ret['activity']['notes']):'Retorno agendado sem observação.'?></p></div>
         <span class="<?=$retLate?'late':e((string)$ret['status'])?>"><?=$retLate?'Atrasado':($ret['status']==='pending'?'Pendente':($ret['status']==='done'?'Concluído':'Cancelado'))?></span>
        </article>
       <?php endforeach;?>
       <?php if(!$returnRows):?><div class="tdf-empty"><i class="fa-regular fa-calendar-xmark"></i><strong>Nenhum retorno registrado</strong></div><?php endif;?>
      </div>
     </section>
    </div>

    <div class="tdf-tab-panel" data-tdf-panel="sales">
     <section class="tdf-card tdf-full">
      <header><div><strong>Vendas</strong><small>Registros de atividade classificados como venda nesta Inteligência Comercial.</small></div><span><?=$saleCount?> venda<?=$saleCount===1?'':'s'?></span></header>
      <div class="tdf-sales-list">
       <?php foreach($saleActivities as $sale):$saleTs=strtotime((string)$sale['created_at']);?>
        <article><span><i class="fa-solid fa-cart-shopping"></i></span><div><strong>Venda registrada</strong><p><?=!empty($sale['notes'])?nl2br(e((string)$sale['notes'])):'Sem detalhes adicionais.'?></p><small><?=e((string)($sale['user_name']??'Usuário'))?> · <?=date('d/m/Y H:i',$saleTs)?></small></div></article>
       <?php endforeach;?>
       <?php if(!$saleActivities):?><div class="tdf-empty"><i class="fa-solid fa-cart-shopping"></i><strong>Nenhuma venda registrada nesta ficha</strong><span>A estrutura específica de vendas será evoluída na Tela 6.</span></div><?php endif;?>
      </div>
     </section>
    </div>

    <div class="tdf-tab-panel" data-tdf-panel="observations">
     <section class="tdf-card tdf-full">
      <header><div><strong>Observações</strong><small>Notas livres de contexto. Não contam como atividade e não alteram a prioridade da carteira.</small></div><span><?=$noteCount?> nota<?=$noteCount===1?'':'s'?></span></header>
      <?php if($canWork):?><form class="tdf-note-form" method="post" action="<?=APP_URL?>/commercial/accounts/<?=rawurlencode((string)$account['omie_code'])?>/note"><input type="hidden" name="_token" value="<?=CSRF::token()?>"><textarea name="note" rows="4" maxlength="10000" placeholder="Ex.: parceiro prefere contato pelo WhatsApp e costuma realizar pedidos na primeira semana do mês." required></textarea><button type="submit"><i class="fa-solid fa-plus"></i>Adicionar observação</button></form><?php endif;?>
      <div class="tdf-note-list">
       <?php foreach($commercialNotes as $note):?><article><p><?=nl2br(e((string)$note['note']))?></p><footer><b><?=e((string)$note['user_name'])?></b><time><?=date('d/m/Y H:i',strtotime((string)$note['created_at']))?></time></footer></article><?php endforeach;?>
       <?php if(!$commercialNotes):?><div class="tdf-empty"><i class="fa-regular fa-note-sticky"></i><strong>Nenhuma observação registrada</strong></div><?php endif;?>
      </div>
     </section>
    </div>
     ...
     <form method="post" action="<?=APP_URL?>/commercial/accounts/<?=rawurlencode((string)$account['omie_code'])?>/profile">
      ...
      <div class="tdf-classification-body">
       <div class="tdf-class-options">
        <label><input type="checkbox" name="is_cfc" value="1" <?=$isCfc?'checked':''?>><span><i class="fa-solid fa-building-columns"></i><b>CFC</b></span></label>
        <label><input type="checkbox" name="is_reseller" value="1" <?=$isReseller?'checked':''?>><span><i class="fa-solid fa-handshake"></i><b>Revendedor</b></span></label>
       </div>
       <section class="tdf-audit"><h3>Últimas alterações</h3>
        <?php foreach(array_slice($classificationAudit,0,6) as $change):?><p><b><?=date('d/m',strtotime((string)$change['created_at']))?></b>, <?=e((string)($change['actor_name']??'Sistema'))?> alterou de <strong><?=e($classificationFromRaw($change['previous_value']??null))?></strong> para <strong><?=e($classificationFromRaw($change['new_value']??null))?></strong>.</p><?php endforeach;?>
        <?php if(!$classificationAudit):?><p class="muted">Ainda não há alterações manuais de classificação.</p><?php endif;?>
       </section>
      </div>
      <footer><button type="button" class="secondary" data-tdf-classification-close>Cancelar</button><button type="submit" class="primary"><i class="fa-solid fa-floppy-disk"></i>Salvar classificação</button></footer>
     </form>
    </dialog>
    <?php endif;?>
    ...
    <?php render_commercial_activity_dialog([
     'user'=>$u,'types'=>$activityTypes??[],'channels'=>$activityChannels??[],'categories'=>$activityCategories??[],'assignable'=>$activityAssignableUsers??[],
     'return_to'=>'account','return_query'=>''
    ]);?>
   </section>
  <?php break;

  case 'commercial_partners':
   $partnerRows=$partners['rows']??[];$partnerFilters=$partners['filters']??[];$partnerMetrics=$partners['metrics']??[];
   $partnerMetricUrl=static function(string $status)use($partnerFilters): string{$query=['status'=>$status];if(!empty($partnerFilters['q']))$query['q']=(string)$partnerFilters['q'];if($status==='all')unset($query['status']);return APP_URL.'/commercial-partners'.($query?'?'.http_build_query($query):'');};
   ?>
   <section class="cix-page">
    ...
    <nav class="cix-partner-metrics" aria-label="Indicadores e filtros dos parceiros">
     <?php foreach([
      ['all','Total de parceiros','Toda a carteira','fa-people-group','blue','total'],
      ['active','Ativados','Contato nos últimos 14 dias','fa-circle-check','green','active'],
      ['activation','Em ativação','Entre 15 e 30 dias','fa-bolt','amber','activation'],
      ['reactivation','Precisam reativação','Mais de 30 dias ou nunca','fa-triangle-exclamation','danger','reactivation'],
      ['never','Nunca trabalhados','Sem trabalho registrado','fa-user-clock','purple','never']
     ] as $metric):$metricActive=($partnerFilters['status']??'all')===$metric[0];?>
      <a class="<?=$metric[4]?> <?=$metricActive?'active':''?>" href="<?=e($partnerMetricUrl($metric[0]))?>" <?=$metricActive?'aria-current="page"':''?>>
       <span><i class="fa-solid <?=$metric[3]?>"></i></span><div><small><?=e($metric[1])?></small><strong><?=number_format((int)($partnerMetrics[$metric[5]]??0),0,',','.')?></strong><em><?=e($metric[2])?></em></div><i class="fa-solid fa-arrow-right"></i>
      </a>
     <?php endforeach;?>
    </nav>
     ...
     <form method="post" action="<?=APP_URL?>/commercial-partners/sync-apply" data-partner-sync-form>
      ...
      <div class="cix-partner-sync-content">
       ...
       <div class="cix-partner-sync-table-wrap" data-partner-sync-table hidden>
        <table class="cix-partner-sync-table" data-partner-sync-datatable data-no-datatable>
  ...
  case 'commercial_management':
   ...
   <section class="cix-page cix-management"><header class="cix-head"><div><span class="cix-icon"><i class="fa-solid fa-chart-column"></i></span><div><small>GESTÃO</small><h1>Painel da Gestão</h1><p>O que foi produzido, por quem, quando e com qual resultado.</p></div></div><a class="cix-btn" href="<?=APP_URL?>/commercial-management?<?=e(http_build_query(array_merge($_GET,['export'=>'csv'])))?>"><i class="fa-solid fa-download"></i>Exportar</a></header>
    <form class="cix-filters management cix-management-filters" method="get" data-management-filters>
     <label><span>Período</span><select class="form-select" name="period" data-management-period><option value="today" <?=$mf['period']==='today'?'selected':''?>>Hoje</option><option value="yesterday" <?=$mf['period']==='yesterday'?'selected':''?>>Ontem</option><option value="week" <?=$mf['period']==='week'?'selected':''?>>Semana</option><option value="month" <?=$mf['period']==='month'?'selected':''?>>Mês</option><option value="custom" <?=$mf['period']==='custom'?'selected':''?>>Personalizado</option></select></label>
     <label><span>Consultora</span><select class="form-select" name="seller_id" data-management-auto-filter><option value="0">Todas as consultoras</option><?php foreach($commercialSellers as $seller):?><option value="<?=$seller['id']?>" <?=$mf['seller']==$seller['id']?'selected':''?>><?=e($seller['name'])?></option><?php endforeach;?></select></label>
     <label><span>Tipo de atividade</span><select class="form-select" name="activity_type" data-management-auto-filter><option value="all">Todas as atividades</option><?php foreach($activityLabels as $code=>$label):?><option value="<?=e($code)?>" <?=$mf['activity']===$code?'selected':''?>><?=e($label)?></option><?php endforeach;?></select></label>
     <label><span>Tipo de cliente</span><select class="form-select" name="classification" data-management-auto-filter><option value="all">Todos</option><option value="cfc" <?=$mf['classification']==='cfc'?'selected':''?>>CFC</option><option value="reseller" <?=$mf['classification']==='reseller'?'selected':''?>>Revendedor</option><option value="both" <?=$mf['classification']==='both'?'selected':''?>>CFC + Revendedor</option></select></label>
     <label><span>Tipo de venda</span><select class="form-select" name="sale_type" data-management-auto-filter><option value="all">Todos</option><?php foreach($saleTypes as $code=>$label):?><option value="<?=e($code)?>" <?=$mf['saleType']===$code?'selected':''?>><?=e($label)?></option><?php endforeach;?></select></label>
     <label><span>Canal</span><select class="form-select" name="channel" data-management-auto-filter><option value="all">Todos os canais</option><?php foreach($activityChannels as $channel):?><option value="<?=e($channel['code'])?>" <?=$mf['channel']===$channel['code']?'selected':''?>><?=e($channel['label'])?></option><?php endforeach;?></select></label>
     <a class="cix-management-clear" href="<?=APP_URL?>/commercial-management"><i class="fa-solid fa-filter-circle-xmark"></i>Limpar filtros</a>
     <div class="cix-management-custom-period" data-management-custom-period<?=$mf['period']==='custom'?'':' hidden'?>>
      <label><span>Data inicial</span><input class="form-control" type="date" name="start" value="<?=e($mf['start'])?>"></label>
      <label><span>Data final</span><input class="form-control" type="date" name="end" value="<?=e(date('Y-m-d',strtotime($mf['end'].' -1 day')))?>"></label>
      <button class="cix-btn primary" type="submit"><i class="fa-solid fa-check"></i>Aplicar período</button>
     </div>
    </form>
  ...
  case 'clients':?>
   ...
   <section class="tdc-page <?=$portfolioMode?'tdc-portfolio-page':''?>">
    ...
    <nav class="tdc-hub-nav" aria-label="Central de clientes">
     ...
     <a class="<?=$clientSegment==='general'&&$crmStatus==='active'?'active':''?>" href="<?=APP_URL?>/clients?segment=general"><i class="fa-solid fa-briefcase"></i><span><strong>Comercial</strong><small><?=number_format((int)($baseCounts['general']??0),0,',','.')?> ativos</small></span></a>
     <a class="<?=$clientSegment==='ead_reciclagem'&&$crmStatus==='active'?'active':''?>" href="<?=APP_URL?>/clients?segment=ead_reciclagem"><i class="fa-solid fa-graduation-cap"></i><span><strong>EAD Reciclagem</strong><small><?=number_format((int)($baseCounts['ead_reciclagem']??0),0,',','.')?> ativos</small></span></a>
     <a class="<?=$clientSegment==='suporte_pet'&&$crmStatus==='active'?'active':''?>" href="<?=APP_URL?>/clients?segment=suporte_pet"><i class="fa-solid fa-paw"></i><span><strong>Suporte PET</strong><small><?=number_format((int)($baseCounts['suporte_pet']??0),0,',','.')?> ativos</small></span></a>
    ...
    <div class="tdc-kpis">
     <article class="tdc-kpi green"><span class="tdc-kpi-icon"><i class="fa-solid fa-address-book"></i></span><div><small><?=$portfolioMode?'Clientes vinculados':($crmStatus==='inactive'?'Inativos no CRM':($clientSegment==='all'?'Ativos na base completa':'Clientes nesta visão'))?></small><strong><?=number_format((int)$clientStats['total'],0,',','.')?></strong><p><?php if($portfolioMode):?>Somente sua responsabilidade<?php elseif(!empty($clientTagsSelected)||!empty($clientUfs)||$q!==''||$sellerFilter!==''):?>Filtro atual · base ativa total <?=number_format((int)($baseCounts['all']??0),0,',','.')?><?php elseif($clientSegment==='all'):?><?=number_format((int)($baseCounts['stored']??0),0,',','.')?> armazenados · <?=number_format((int)($baseCounts['crm_inactive']??0),0,',','.')?> inativos no CRM · <?=number_format((int)($baseCounts['inactive']??0),0,',','.')?> inativos na origem<?php else:?>De <?=number_format((int)($baseCounts['all']??0),0,',','.')?> clientes ativos na base completa<?php endif;?></p></div></article>
     ...
     <?php if($portfolioMode):?><article class="tdc-kpi orange owner"><span class="tdc-kpi-icon"><i class="fa-solid fa-user-shield"></i></span><div><small>Responsável pela carteira</small><strong><?=e(Auth::user()['name']??'Consultora')?></strong><p><?=$crmPortfolioReady?'Vínculo oficial do CRM Omie':'Fallback temporário da base anterior'?></p></div></article><?php else:?><article class="tdc-kpi orange"><span class="tdc-kpi-icon"><i class="fa-solid fa-user-tag"></i></span><div><small><?=$crmPortfolioReady?'Sem responsável CRM':'Sem vendedor'?></small><strong><?=number_format((int)$clientStats['without_seller'],0,',','.')?></strong><p><?=$crmPortfolioReady?'Contas sem consultora mapeada':'Clientes para distribuição'?></p></div></article><?php endif;?>
    </div>

    <?php if(Auth::can('admin','supervisor')&&$crmStatus==='active'&&in_array($clientSegment,['all','general'],true)):?>
    <section class="tdc-intelligence" aria-label="Saúde da base de clientes">
     <div class="tdc-intelligence-head"><span><i class="fa-solid fa-brain"></i></span><div><strong>Controle inteligente da base</strong><small><?=$crmPortfolioReady?'A carteira agora obedece ao responsável da Conta no CRM Omie.':'Modo de transição: conclua a carga do CRM Omie para ativar a carteira oficial.'?></small></div></div>
     <div class="tdc-intelligence-items">
      <a href="<?=APP_URL?>/clients-audit?tab=responsibility&month=<?=e($portfolioMonth)?>"><b><?=number_format((int)($clientStats['seller_divergences']??0),0,',','.')?></b><span><?=$crmPortfolioReady?'Responsáveis não mapeados':'Divergências CRM × Omie'?></span><small><?=$crmPortfolioReady?'Conta possui nCodVend, mas falta vínculo com usuário Tecnodata':'Vendedor principal diferente da origem Omie'?></small></a>
      <div><b><?=number_format((int)($crmPortfolioReady?($commercialHealth['linked_accounts']??0):($clientStats['monthly_overrides']??0)),0,',','.')?></b><span><?=$crmPortfolioReady?'Contas CRM vinculadas':'Ajustes em '.$portfolioMonthLabel?></span><small><?=$crmPortfolioReady?'Reconciliação confirmada com cliente Geral':'Carteiras provisórias deste mês'?></small></div>
    ...
    <?php if($clientSegment==='general'&&!$crmPortfolioReady):?>
    ...
    <?php if(Auth::can('seller')):?><div class="tdc-seller-base-note"><i class="fa-solid fa-circle-info"></i><div><strong>Base completa para consulta e manutenção</strong><small>Você pode localizar, visualizar e corrigir qualquer Cliente Omie ativo, inclusive cadastros que ainda precisam de ajuste de integração. Atividades, retornos e agenda comercial continuam restritos à sua carteira oficial de Contas CRM.</small></div></div><?php endif;?>
    <?php endif;?>

    <?php if(Auth::can('admin','supervisor')):$identitySummary=$identitySummary??[];$crmOnlyAccounts=$crmOnlyAccounts??[];?>
    <section class="tdc-identity-hub" aria-label="Identidade comercial">
     <header><div><span><i class="fa-solid fa-diagram-project"></i></span><div><strong>Identidade comercial unificada</strong><small>CRM Omie define a carteira. Cliente Geral Omie complementa vendas, serviços e financeiro.</small></div></div><a href="<?=APP_URL?>/clients-audit"><i class="fa-solid fa-magnifying-glass"></i>Auditar vínculos</a></header>
     <div class="tdc-identity-kpis">
      <a class="<?=$identityFilter==='linked'?'active':''?>" href="<?=APP_URL?>/clients?identity=linked"><small>CRM + Vendas</small><strong><?=number_format((int)($identitySummary['linked']??0),0,',','.')?></strong><span>Identidade completa</span></a>
      <a class="<?=$identityFilter==='sales_only'?'active warning':''?>" href="<?=APP_URL?>/clients?identity=sales_only"><small>Somente Vendas</small><strong><?=number_format((int)($identitySummary['sales_only']??0),0,',','.')?></strong><span>Sem Conta CRM ativa</span></a>
      <a class="<?=$identityFilter==='historical'?'active historical':''?>" href="<?=APP_URL?>/clients?identity=historical"><small>CRM histórico</small><strong><?=number_format((int)($identitySummary['historical']??0),0,',','.')?></strong><span>Vínculo preservado/inativo</span></a>
      <div class="crm-only"><small>Somente CRM</small><strong><?=number_format((int)($identitySummary['crm_only']??0),0,',','.')?></strong><span>Sem Cliente Geral vinculado</span></div>
      <div class="<?=!empty($identitySummary['conflicts'])?'danger':''?>"><small>Conflitos</small><strong><?=number_format((int)($identitySummary['conflicts']??0),0,',','.')?></strong><span>Mais de uma Conta CRM ativa</span></div>
     </div>
     <?php if($crmOnlyAccounts):?><details class="tdc-crm-only-preview"><summary><span><i class="fa-solid fa-link-slash"></i>Contas CRM aguardando Cliente Geral</span><b><?=number_format((int)($identitySummary['crm_only']??0),0,',','.')?></b><i class="fa-solid fa-chevron-down"></i></summary><div><?php foreach($crmOnlyAccounts as $crmOnly):?><a href="<?=APP_URL?>/commercial/accounts/<?=rawurlencode((string)$crmOnly['omie_code'])?>"><span><strong><?=e((string)$crmOnly['account_name'])?></strong><small><?=e((string)($crmOnly['document']?:'Documento não informado'))?> · <?=e((string)$crmOnly['owner_name'])?></small></span><em><?=e((string)$crmOnly['omie_code'])?></em><i class="fa-solid fa-arrow-right"></i></a><?php endforeach;?></div></details><?php endif;?>
    </section>
    <?php endif;?>
    ...
    <section class="tdc-table-card">
     ...
     <div class="tdc-list-filterbar">
      ...
      <form method="get" action="<?=APP_URL?>/<?=$portfolioMode?'my-portfolio':'clients'?>">
       ...
       <label><span><i class="fa-solid fa-tags"></i> Classificação comercial</span><select class="form-select" name="classification" onchange="this.form.submit()"><option value="all" <?=$classificationFilter==='all'?'selected':''?>>Todas</option><option value="cfc" <?=$classificationFilter==='cfc'?'selected':''?>>CFC</option><option value="reseller" <?=$classificationFilter==='reseller'?'selected':''?>>Revendedor</option><option value="both" <?=$classificationFilter==='both'?'selected':''?>>CFC + Revendedor</option><option value="unclassified" <?=$classificationFilter==='unclassified'?'selected':''?>>Sem classificação</option></select></label>
     ...
     <?php if(Auth::can('admin','supervisor')&&$crmStatus==='active'&&!$crmPortfolioReady):?>
     ...
     <div class="table-card tdc-table-wrap">
      ...
      <table class="table tdc-table clients-datatable" data-server-url="<?=APP_URL?>/api/clients/datatable?<?=e(http_build_query($clientDataParams))?>" data-search="<?=e($q)?>" data-page-length="10" data-length-change="1" data-order-column="<?=$portfolioMode?5:(Auth::can('admin','supervisor')&&$crmStatus==='active'?1:0)?>" data-order-direction="asc">
       ...
       <tbody><?php foreach($rows as $r):?><tr>
        ...
        <td><span class="tdc-integration linked"><span><i class="fa-solid fa-link"></i>CRM + Vendas</span><small>Identidade comercial conectada</small></span></td>
      ...
      <form method="post" action="<?=$editClient?APP_URL.'/clients/'.(int)$editClient['id'].'/update':APP_URL.'/clients/save-local'?>" id="clientCreateForm" class="tdc-form">
       ...
       <section class="tdc-section" id="tdc-identificacao">
        ...
        <div class="tdc-fields">
         ...
         <div class="field span-4"><label>E-mail<span class="tdc-required">*</span></label><input class="form-control" type="text" inputmode="email" autocomplete="email" name="email" maxlength="1000" value="<?=e((string)($old['email']??''))?>" placeholder="email1@empresa.com, email2@empresa.com" required><small class="tdc-hint">Mais de um e-mail é válido. Separe os endereços por vírgula.</small></div>
       ...
       <section class="tdc-section" id="tdc-comercial">
        ...
        <div class="tdc-fields">
         ...
         <div class="field span-12"><label>Observações</label><textarea class="form-control" name="notes" rows="5" maxlength="10000" placeholder="Informações úteis para o atendimento."><?=e((string)($old['notes']??''))?></textarea></div>
        </div>
       </section>
  ...
  case 'client':
   ...
   <section class="tdc-page">
    <header class="tdc-head tdc-detail-head">
     <div class="tdc-head-main"><span class="tdc-detail-avatar"><?=e(mb_strtoupper(mb_substr((string)$client['name'],0,1)))?></span><div><a class="tdc-back" href="<?=APP_URL?>/clients<?=!empty($sharedUnassigned)?'?scope=unassigned':''?>"><i class="fa-solid fa-arrow-left"></i>Carteira de clientes</a><h1><?=e($client['name'])?></h1><?php $detailOmieCreated=(string)($client['omie_created_at']??'');if($detailOmieCreated==='')$detailOmieCreated=(string)(ClientService::omieCreatedAtFromRaw($client['raw_json']??null)??'');$detailInclusion=$detailOmieCreated!==''?$detailOmieCreated:(string)($client['created_at']??$client['updated_at']??'');?><div class="tdc-meta"><span><i class="fa-solid fa-location-dot"></i><?=e(trim(($client['city']??'').' / '.($client['uf']??''),' /')?:'Localização não informada')?></span><span><i class="fa-regular fa-id-card"></i><?=e($client['document']?:'Documento não informado')?></span><span><i class="fa-solid fa-cloud"></i><?=$isLocal?'Somente local':'Omie '.e($client['omie_code'])?></span><span title="<?=$detailInclusion!==''?'Data de inclusão identificada no '.($detailOmieCreated!==''?'Omie':'CRM'):''?>"><i class="fa-regular fa-calendar-plus"></i><?=$detailInclusion!==''?'Incluído '.date('d/m/Y',strtotime($detailInclusion)).' · '.($detailOmieCreated!==''?'Omie':'CRM'):'Inclusão não identificada'?></span></div></div></div>
     <div class="tdc-head-actions"><?php if($operationalAccess):?><button class="tdc-btn tdc-btn-schedule" type="button" data-global-task-open data-task-client-id="<?=$client['id']?>" data-task-client-name="<?=e($client['name'])?>" data-task-context="sales"><i class="fa-regular fa-calendar-plus"></i>Nova tarefa</button><?php endif;?><?php if(empty($sharedUnassigned)&&!$crmInactive&&!$originInactive):?><a class="tdc-btn" href="<?=APP_URL?>/clients/<?=$client['id']?>/edit"><i class="fa-regular fa-pen-to-square"></i>Editar</a><?php endif;?><?php if(Auth::can('admin','supervisor')):?><button class="tdc-btn tdc-btn-omie-check" type="button" data-client-omie-check data-client-id="<?=$client['id']?>"><i class="fa-solid fa-cloud-arrow-down"></i>Consultar Omie</button><?php if(!$originInactive):?><form method="post" action="<?=APP_URL?>/clients/<?=$client['id']?>/crm-status"><input type="hidden" name="_token" value="<?=CSRF::token()?>"><input type="hidden" name="inactive" value="<?=$crmInactive?'0':'1'?>"><button class="tdc-btn <?=$crmInactive?'tdc-btn-reactivate':'tdc-btn-inactivate'?>" type="submit" data-confirm="<?=$crmInactive?'Reativar este cliente no CRM e devolvê-lo às carteiras e buscas operacionais?':'Inativar este cliente somente no CRM? Ele deixará de aparecer para vendedores e cobrança, sem alterar a Omie.'?>"><i class="fa-solid <?=$crmInactive?'fa-user-check':'fa-user-slash'?>"></i><?=$crmInactive?'Reativar no CRM':'Inativar no CRM'?></button></form><?php endif;?><?php endif;?><?php if($operationalAccess&&sales_flow_enabled()&&!$isLocal):?><a class="tdc-btn" href="<?=APP_URL?>/opportunities?client_id=<?=$client['id']?>"><i class="fa-solid fa-chart-column"></i>Oportunidade</a><?php endif;?><?php if($operationalAccess&&!$isLocal):?><a class="tdc-btn tdc-btn-primary" href="<?=APP_URL?>/orders/new?client_id=<?=$client['id']?>"><i class="fa-solid fa-plus"></i>Novo pedido</a><?php endif;?></div>
    </header>
    ...
    <?php endif;?>
    <?php if(($loggedUser['role']??'')==='seller'&&!$operationalAccess):?><div class="tdc-sync shared"><span class="icon"><i class="fa-solid fa-user-lock"></i></span><div><strong>Cadastro liberado para correção</strong><p>Você pode consultar este cliente, mas as ações comerciais pertencem a <?=e((string)(!empty($crmPortfolioReady)?($crmOwnerName??$crmOwnerRemoteName??'outro responsável'):$effectiveSellerName))?>.</p></div></div><?php endif;?>
    ...
    <div class="tdc-detail-grid">
     <section class="tdc-card">
      ...
      <div class="tdc-info-grid">
       ...
       <div><span>CPF / CNPJ</span><strong><?=e($formData['document']?:'—')?></strong></div><div><span>E-mail<?=count(ClientService::emailList($formData['email']??''))>1?'s':''?></span><?php $detailEmails=ClientService::emailList($formData['email']??'');if($detailEmails):?><div class="tdc-email-list"><?php foreach($detailEmails as $detailEmail):?><a href="mailto:<?=e($detailEmail)?>"><i class="fa-regular fa-envelope"></i><?=e($detailEmail)?></a><?php endforeach;?></div><?php else:?><strong>—</strong><?php endif;?></div>
       <div><span>Telefone</span><?php $detailPhone=trim(($formData['phone_ddd']?:'').' '.($formData['phone_number']?:''));if($detailPhone!==''):?><a href="#" data-baldussi-phone="<?=e($detailPhone)?>" data-baldussi-account-code="<?=e((string)($client['crm_account_code']??''))?>" data-baldussi-account-name="<?=e((string)($crmAccount['trade_name']??$crmAccount['name']??$client['name']??'Cliente'))?>"><strong><i class="fa-solid fa-phone"></i> <?=e($detailPhone)?></strong></a><?php else:?><strong>—</strong><?php endif;?></div><div><span>Período de compra</span><strong><?=!empty($client['first_purchase_at'])?brdate((string)$client['first_purchase_at']).' → '.brdate((string)($client['last_purchase_at']??$client['first_purchase_at'])):'Sem compra identificada'?></strong><small>Pedidos e serviços válidos</small></div><div><span>Vendedor principal</span><strong><?=e((string)$principalSellerName)?></strong><small>Carteira efetiva: <?=e((string)$effectiveSellerName)?></small></div>
     ...
     <section class="tdc-card">
      ...
      <?php elseif($crmInactive):?>
       <div class="tdc-contact-locked crm-inactive"><i class="fa-solid fa-user-slash"></i><strong>Cliente fora da operação</strong><p>Reative o cliente no CRM para registrar contatos, tarefas, oportunidades ou novos pedidos.</p></div>
      <?php else:?>
       <div class="tdc-contact-locked"><i class="fa-solid fa-user-lock"></i><strong>Atendimento de outra carteira</strong><p>O cliente pertence a <?=e((string)(!empty($crmPortfolioReady)?($crmOwnerName??$crmOwnerRemoteName??'outro responsável'):$effectiveSellerName))?>. Consulte o histórico normalmente; novas ações comerciais devem ser registradas pela carteira responsável.</p></div>
      <?php endif;?>
     </section>
    </div>
    ...
    <section class="tdc-card tdc-commercial-audit">
     ...
     <div class="tdc-commercial-audit-list">
      <?php foreach(array_slice($commercialAudit,0,15) as $change):$field=(string)($change['field_name']??'');$old=json_decode((string)($change['previous_value']??''),true);$new=json_decode((string)($change['new_value']??''),true);?>
       <div><span class="icon"><i class="fa-solid <?=$field==='responsible'?'fa-user-shield':($field==='strategic_notes'?'fa-note-sticky':'fa-tags')?>"></i></span><div><strong><?=$field==='classification'?'Classificação alterada':($field==='responsible'?'Responsável da carteira alterado':'Observação estratégica alterada')?></strong>
       <?php if($field==='classification'&&is_array($old)&&is_array($new)):?><p><?=(!empty($old['cfc'])||!empty($old['reseller']))?trim((!empty($old['cfc'])?'CFC ':'').(!empty($old['reseller'])?'Revendedor':'')):'Sem classificação'?> <i class="fa-solid fa-arrow-right"></i> <?=(!empty($new['cfc'])||!empty($new['reseller']))?trim((!empty($new['cfc'])?'CFC ':'').(!empty($new['reseller'])?'Revendedor':'')):'Sem classificação'?></p>
       <?php elseif($field==='responsible'&&is_array($old)&&is_array($new)):?><p><?=e((string)($old['name']??'Sem responsável'))?> <i class="fa-solid fa-arrow-right"></i> <?=e((string)($new['name']??'Sem responsável'))?></p>
       <?php elseif($field==='strategic_notes'):?><p><?=e(mb_strimwidth((string)($change['previous_value']??''),0,90,'…'))?> <i class="fa-solid fa-arrow-right"></i> <?=e(mb_strimwidth((string)($change['new_value']??''),0,90,'…'))?></p><?php endif;?>
       <small><?=e((string)($change['actor_name']??'Sistema'))?> · <?=date('d/m/Y H:i',strtotime((string)$change['created_at']))?> · <?=e((string)($change['source']??'tecnodata'))?> · <?=e((string)($change['sync_status']??''))?></small></div></div>
      <?php endforeach;?>
     </div>
    </section>
    <?php endif;?>
     ...
     <div class="omie-tab-content">
      ...
      <section class="order-tab-panel" data-order-panel="installments">
       ...
       <div class="omie-installments-wrap">
        <table class="omie-installments-table" data-no-datatable>
...
window.ORDER_META=<?=json_encode(['categories'=>$categories,'taxes'=>$taxes,'stocks'=>$stocks,'profiles'=>$profiles,'paymentMethods'=>$methods,'documentTypes'=>$documents,'defaults'=>$d],JSON_UNESCAPED_UNICODE)?>;
  ...
  case 'collection':
   ...
   <section class="tdcob4-page">
    ...
    <?php if(!empty($flash)):?><div class="alert alert-<?=e($flash['type']??'success')?>"><?=e($flash['message']??'')?></div><?php endif;?>
    <div class="tdcob4-identity-note"><i class="fa-solid fa-link"></i><div><strong>Cobrança conectada à identidade comercial</strong><span>Esta carteira mostra somente Cliente Geral Omie com Conta CRM ativa vinculada. Assim cobrança, consultora, vendas e histórico trabalham sobre o mesmo cliente.</span><?php if(Auth::can('admin','supervisor')&&!empty($collectionIdentityPending)):?><a href="<?=APP_URL?>/clients?identity=sales_only"><b><?=number_format((int)$collectionIdentityPending,0,',','.')?></b> cobrança(s) estão fora da operação por falta de Conta CRM ativa — revisar identidade.</a><?php endif;?></div></div>
  ...
  case 'collection_report':
   $reportSummary=$collectionReportSummary??[];$reportMonth=(string)($collectionReportMonth??date('Y-m'));$reportStatus=(string)($collectionReportStatus??'all');?>
     ...
     <div class="table-card tdcob4-table-wrap"><table class="table tdcob4-table" data-page-length="10" data-length-change="1" data-order-column="2" data-order-direction="desc">
      ...
      <tbody><?php foreach($collectionReportRows??[] as $row):$paid=(float)$row['recovered']>0;$contacted=(int)$row['action_count']>0;$hasRecord=!empty($row['last_action_at']);?>
       <tr>
        ...
        <td class="tdcob4-report-event" data-order="<?=$hasRecord?strtotime((string)$row['last_action_at']):0?>"><?php if($hasRecord):?><strong><?=date('d/m/Y H:i',strtotime((string)$row['last_action_at']))?></strong><small><?=e(contact_channel_label((string)$row['last_channel']))?> · <?=e(task_result_label((string)$row['last_result']))?><?php if((int)$row['action_count']>0):?> · <?=number_format((int)$row['action_count'],0,',','.')?> contato(s) no mês<?php endif;?></small><?php else:?><span class="tdcob4-report-empty">Sem ação registrada no mês</span><?php endif;?></td>
    ...
    <div class="tdcob-recovery-grid">
     <section class="tdcob-card">
      ...
      <form method="post" action="<?=APP_URL?>/collection/recoveries" class="tdcob-recovery-form" data-collection-recovery-form>
       ...
       <label>Observação<textarea class="form-control" name="notes" rows="4" maxlength="5000" placeholder="Opcional"><?=e((string)($old['notes']??''))?></textarea></label>
   ...
   <section class="tdcob-page tdcob4-case-page">
    ...
    <section class="tdcob-card tdcob-action-card">
     ...
     <form method="post" action="<?=APP_URL?>/collection/<?=$case['client_id']?>/action" class="tdcob-form tdcob-action-form">
      ...
      <label>Canal<select class="form-select" name="channel"><?php foreach($contactChannels??[] as $channelOption):?><option value="<?=e($channelOption['code'])?>"><?=e($channelOption['label'])?></option><?php endforeach;?></select></label>
      ...
      <label class="notes">Anotação<textarea class="form-control" name="notes" rows="4" maxlength="10000" placeholder="Resumo do contato ou condição negociada"></textarea></label>
  ...
  case 'agenda':
   ...
   $agendaLate=(int)($stats['late_count']??0);$agendaTodayCount=(int)($stats['today_count']??0);$agendaUpcoming=(int)($stats['upcoming_count']??0);$agendaCollection=(int)($stats['collection_count']??0);$agendaTotal=(int)($stats['total']??0);
   $agendaRoleLabels=['seller'=>'Comercial','collector'=>'Cobrança','supervisor'=>'Supervisor','admin'=>'Admin'];
   $groups=['late'=>[],'today'=>[],'upcoming'=>[]];
   foreach($rows as $agendaRow){$dueDate=date('Y-m-d',strtotime((string)$agendaRow['due_at']));if($dueDate<date('Y-m-d'))$groups['late'][]=$agendaRow;elseif($dueDate===date('Y-m-d'))$groups['today'][]=$agendaRow;else $groups['upcoming'][]=$agendaRow;}
   ...
   <section class="tdw-page tdw-agenda">
    <header class="tdw-page-head tdw-agenda-head">
     ...
     <div class="tdw-head-actions"><button class="tdw-btn primary" type="button" data-global-task-open data-task-context="<?=$u['role']==='collector'?'collection':'sales'?>"><i class="fa-solid fa-plus"></i>Nova tarefa</button></div>
    </header>

    <?php if(!empty($flash)):?><div class="alert alert-<?=e($flash['type']??'success')?>"><?=e($flash['message']??'')?></div><?php endif;?>

    <nav class="tdw-priority-strip tdw-agenda-strip" aria-label="Situação da agenda">
     <?php foreach([
      'late'=>['Atrasados',$agendaLate,'fa-triangle-exclamation','danger'],
      'today'=>['Hoje',$agendaTodayCount,'fa-clock','warning'],
      'upcoming'=>['Próximos',$agendaUpcoming,'fa-calendar-check','green'],
      'all'=>['Todos pendentes',$agendaTotal,'fa-list-check','neutral']
     ] as $key=>$item):?>
      <a class="tdw-priority <?=$item[3]?> <?=($agendaPeriod??'all')===$key?'active':''?>" href="<?=$agendaUrl(['period'=>$key])?>"><span><i class="fa-solid <?=$item[2]?>"></i></span><div><small><?=$item[0]?></small><strong><?=number_format($item[1],0,',','.')?></strong></div></a>
     <?php endforeach;?>
    </nav>

    <section class="tdw-filter-shell">
     <form method="get" class="tdw-filter-form tdw-agenda-filters">
      <label><span>Período</span><select class="form-select" name="period"><option value="all" <?=($agendaPeriod??'all')==='all'?'selected':''?>>Todos pendentes</option><option value="late" <?=($agendaPeriod??'all')==='late'?'selected':''?>>Atrasados</option><option value="today" <?=($agendaPeriod??'all')==='today'?'selected':''?>>Hoje</option><option value="next7" <?=($agendaPeriod??'all')==='next7'?'selected':''?>>Próximos 7 dias</option><option value="upcoming" <?=($agendaPeriod??'all')==='upcoming'?'selected':''?>>Próximos</option></select></label>
      <label><span>Área</span><select class="form-select" name="type" <?=$u['role']==='collector'?'disabled':''?>><?php if($u['role']!=='collector'):?><option value="all" <?=($agendaType??'all')==='all'?'selected':''?>>Todas</option><option value="sales" <?=($agendaType??'all')==='sales'?'selected':''?>>Comercial</option><?php endif;?><option value="collection" <?=($agendaType??'all')==='collection'?'selected':''?>>Cobrança</option></select><?php if($u['role']==='collector'):?><input type="hidden" name="type" value="collection"><?php endif;?></label>
      <?php if(!empty($teamAgenda)):?><label><span>Responsável</span><select class="form-select" name="user_id"><option value="0">Toda a equipe</option><?php foreach($agendaUsers??[] as $agendaUser):?><option value="<?=(int)$agendaUser['id']?>" <?=((int)($agendaFilterUser??0)===(int)$agendaUser['id'])?'selected':''?>><?=e($agendaUser['name'])?> · <?=e($agendaRoleLabels[$agendaUser['role']]??$agendaUser['role'])?></option><?php endforeach;?></select></label><?php endif;?>
      <label><span>Criada em</span><input class="form-control" type="date" name="created_date" value="<?=e($agendaCreatedDate??'')?>"></label>
      <div class="tdw-filter-actions"><button class="tdw-btn primary" type="submit"><i class="fa-solid fa-sliders"></i>Aplicar</button><a class="tdw-icon-btn" href="<?=APP_URL?>/agenda" title="Limpar filtros"><i class="fa-solid fa-rotate-left"></i></a></div>
     </form>
    </section>

    <div class="tdw-agenda-layout">
     <main class="tdw-agenda-main">
      <?php foreach(['late'=>['Atrasados','Compromissos que já passaram do prazo.','danger'],'today'=>['Hoje','O que precisa ser executado hoje.','warning'],'upcoming'=>['Próximos','Retornos já planejados para os próximos dias.','green']] as $groupKey=>$groupMeta):
       if(($agendaPeriod??'all')!=='all'&&($agendaPeriod??'all')!=='next7'&&($agendaPeriod??'all')!==$groupKey)continue;?>
       <section class="tdw-agenda-group <?=$groupMeta[2]?>">
        <header><div><span><i class="fa-solid <?=$groupKey==='late'?'fa-triangle-exclamation':($groupKey==='today'?'fa-clock':'fa-calendar-check')?>"></i></span><div><strong><?=$groupMeta[0]?></strong><small><?=$groupMeta[1]?></small></div></div><b><?=count($groups[$groupKey])?></b></header>
        <div class="tdw-task-list">
         <?php foreach($groups[$groupKey] as $r):
          $due=strtotime((string)$r['due_at']);$created=strtotime((string)$r['created_at']);$isCollection=($r['type']??'')==='collection';
          $taskAccountCode=trim((string)($r['crm_account_code']??''));$taskHref=$isCollection?APP_URL.'/collection/'.(int)$r['client_id']:($taskAccountCode!==''?APP_URL.'/commercial/accounts/'.rawurlencode($taskAccountCode):APP_URL.'/clients/'.(int)$r['client_id']);
          $taskTypeCode=(string)($r['task_type_code']??'');$taskTypeName=$taskTypeLabels[$taskTypeCode]??'Retorno / outro';
         ?>
          <article class="tdw-task">
         ...
         <?php endforeach;?>
         <?php if(empty($groups[$groupKey])):?><div class="tdw-empty compact"><span><i class="fa-solid fa-circle-check"></i></span><strong>Nada em <?=$groupMeta[0]?></strong><p>Não há compromissos nesta faixa para os filtros atuais.</p></div><?php endif;?>
        </div>
       </section>
      <?php endforeach;?>
     </main>

     <aside class="tdw-agenda-side">
      <?php if(!empty($teamAgenda)):?>
       <section class="tdw-panel tdw-team-load"><header class="tdw-panel-head"><div><strong>Carga da equipe</strong><small>Pendências por responsável.</small></div></header><div><?php foreach($agendaWorkload??[] as $work):$initial=mb_strtoupper(mb_substr((string)$work['name'],0,1));?><a href="<?=$agendaUrl(['user_id'=>(int)$work['id']])?>" class="<?=((int)($agendaFilterUser??0)===(int)$work['id'])?'active':''?>"><span class="tdw-mini-avatar"><?=$initial?></span><div><strong><?=e((string)$work['name'])?></strong><small><?=e($agendaRoleLabels[$work['role']]??$work['role'])?></small></div><p><b><?=(int)$work['late_count']?></b> atras. <b><?=(int)$work['today_count']?></b> hoje</p><em><?=(int)$work['total']?></em></a><?php endforeach;?><?php if(empty($agendaWorkload)):?><div class="tdw-empty compact"><strong>Equipe sem pendências</strong><p>Nenhuma tarefa para os filtros atuais.</p></div><?php endif;?></div></section>
      <?php endif;?>
      <section class="tdw-panel tdw-agenda-summary"><header class="tdw-panel-head"><div><strong>Resumo operacional</strong><small>Leitura rápida da fila atual.</small></div></header><dl><div><dt>Atrasados</dt><dd class="danger"><?=$agendaLate?></dd></div><div><dt>Hoje</dt><dd class="warning"><?=$agendaTodayCount?></dd></div><div><dt>Próximos</dt><dd class="green"><?=$agendaUpcoming?></dd></div><div><dt>Cobrança</dt><dd><?=$agendaCollection?></dd></div></dl></section>
     </aside>
    </div>

    <dialog class="tda-task-modal tdw-task-modal" data-agenda-task-modal>
     <form data-agenda-task-form>
      <input type="hidden" name="_token" value="<?=CSRF::token()?>"><input type="hidden" name="mode" value="view" data-agenda-task-mode>
      <header><span data-agenda-task-icon><i class="fa-solid fa-eye"></i></span><div><small>AGENDA / TAREFA</small><strong data-agenda-task-heading>Tarefa completa</strong><p><b data-agenda-task-client>Carregando...</b><em data-agenda-task-status></em></p></div><button type="button" data-agenda-task-close aria-label="Fechar"><i class="fa-solid fa-xmark"></i></button></header>
      <div class="tda-task-body">
       <section class="tda-task-client-card"><div class="tda-task-client-identity"><span><i class="fa-regular fa-building"></i></span><div><small>CONTA / CLIENTE</small><strong data-agenda-task-client-name>—</strong><p data-agenda-task-client-meta>—</p></div></div><div class="tda-task-client-insights" aria-label="Resumo da tarefa"><article><small>Área</small><strong data-agenda-task-card-context>—</strong></article><article><small>Tipo</small><strong data-agenda-task-card-type>—</strong></article><article><small>Responsável</small><strong data-agenda-task-card-assigned>—</strong></article><article><small>Prazo</small><strong data-agenda-task-card-due>—</strong></article></div><a href="#" data-agenda-task-client-link><i class="fa-solid fa-arrow-up-right-from-square"></i>Abrir cadastro</a></section>
       <div class="tda-task-summary"><div class="created"><small>Criada em</small><strong data-agenda-task-created>—</strong></div><div class="creator"><small>Criada por</small><strong data-agenda-task-created-by>—</strong></div><div class="status"><small>Status</small><strong data-agenda-task-status-label>—</strong></div><div class="updated"><small>Última alteração</small><strong data-agenda-task-updated>—</strong></div></div>
       <div class="tda-task-grid"><label><span>Área</span><select class="form-select" name="context" data-agenda-task-context><option value="sales">Comercial</option><option value="collection">Cobrança</option></select></label><label><span>Tipo de tarefa</span><select class="form-select" name="task_type_code" data-agenda-task-type></select></label><label><span>Responsável</span><select class="form-select" name="assigned_user_id" data-agenda-task-assigned></select></label><label><span>Data e hora</span><input class="form-control" type="datetime-local" name="due_at" data-agenda-task-due></label><label class="wide"><span>Descrição da tarefa</span><textarea class="form-control" name="title" rows="5" maxlength="2000" data-agenda-task-description></textarea></label></div>
       <section class="tda-task-completion" data-agenda-task-completion hidden><header><span><i class="fa-solid fa-check"></i></span><div><strong>Conclusão da tarefa</strong><small>Registre o resultado e complemente a informação antes de encerrar.</small></div></header><div class="tda-task-grid"><label><span>Resultado do atendimento <small>opcional</small></span><select class="form-select" name="completion_result_code" data-agenda-task-result><option value="">Somente concluir tarefa</option></select></label><label class="wide"><span>Observação de conclusão <small>opcional</small></span><textarea class="form-control" name="completion_notes" rows="6" maxlength="10000" placeholder="Ex.: cliente confirmou recebimento, proposta encaminhada, retorno concluído..." data-agenda-task-notes></textarea></label></div></section>
      </div>
  ...
  <?php break;
  case 'baldussi_test':
   $bc=$baldussiConfig??[];$bs=$baldussiState??[];$be=(array)($bs['extensions']['rows']??[]);$lastCall=is_array($bs['last_call']??null)?$bs['last_call']:null;$userExtension=trim((string)($baldussiExtension??''));
   ?>
   <section class="tdb-page">
    <header class="tdb-head"><div><span class="tdb-head-icon"><i class="fa-solid fa-phone-volume"></i></span><div><span>COMERCIAL / TELEFONIA</span><h1>Telefonia Baldussi</h1><p>Faça ligações pelo seu ramal e, quando desejar, registre o resultado no histórico do cliente.</p></div></div><?php if(Auth::can('admin','supervisor')):?><a class="tdb-btn" href="<?=APP_URL?>/settings"><i class="fa-solid fa-arrow-left"></i>Configurações</a><?php endif;?></header>
    <?php if($flash):?><div class="alert alert-<?=e($flash['type']??'info')?>"><?=e($flash['message']??'')?></div><?php endif;?>
    <section class="tdb-security"><i class="fa-solid fa-shield-halved"></i><div><strong>Ambiente protegido de testes</strong><p>O token permanece somente no servidor. Senhas SIP e outros campos sensíveis retornados pelo PABX são descartados antes de qualquer dado chegar a esta tela.</p></div></section>
    <div class="tdb-status">
     <article><span><i class="fa-solid fa-cloud"></i></span><div><small>Servidor da API</small><strong><?=e((string)($bc['host']??'Não configurado'))?></strong></div></article>
     <article><span><i class="fa-solid fa-user-shield"></i></span><div><small>Usuário</small><strong><?=e((string)($bc['username_masked']??'Não configurado'))?></strong></div></article>
     <article><span><i class="fa-solid fa-key"></i></span><div><small>Token</small><strong><?=e((string)($bc['token_masked']??'Não configurado'))?></strong></div></article>
     <article class="<?=!empty($bs['tested_at'])?'ok':'pending'?>"><span><i class="fa-solid <?=!empty($bs['tested_at'])?'fa-circle-check':'fa-clock'?>"></i></span><div><small>Último diagnóstico</small><strong><?=!empty($bs['tested_at'])?date('d/m/Y H:i',strtotime((string)$bs['tested_at'])):'Ainda não executado'?></strong></div></article>
    </div>
    <div class="tdb-grid">
     <section class="tdb-card">
      <header><div><span class="tdb-step">1</span><strong>Conexão e ramais</strong><small>Autentica na API e carrega apenas informações operacionais seguras.</small></div></header>
      <div class="tdb-card-body"><p>Este teste não inicia ligações nem altera a configuração do PABX.</p><form method="post" action="<?=APP_URL?>/settings/baldussi/test-connection"><input type="hidden" name="_token" value="<?=CSRF::token()?>"><button class="tdb-btn primary" type="submit" <?=empty($bc['configured'])?'disabled':''?>><i class="fa-solid fa-plug-circle-check"></i>Testar conexão e carregar ramais</button></form></div>
     </section>
     <section class="tdb-card">
    ...
    <?php if($be):?><section class="tdb-card tdb-extensions"><header><div><span class="tdb-step"><i class="fa-solid fa-list"></i></span><strong>Ramais retornados</strong><small><?=number_format((int)($bs['extensions']['returned']??count($be)),0,',','.')?> exibidos de <?=number_format((int)($bs['extensions']['total']??count($be)),0,',','.')?> disponíveis na API.</small></div></header><div class="tdb-table-wrap"><table class="table" data-datatable data-page-length="10"><thead><tr><th>Ramal</th><th>Identificação</th><th>Grupo</th><th>DDD</th><th>Caller ID</th><th>Gravação</th><th>Status</th></tr></thead><tbody><?php foreach($be as $extension):?><tr><td><strong><?=e((string)$extension['number'])?></strong><small>ID <?=e((string)$extension['id'])?></small></td><td><?=e((string)($extension['name']?:'Sem identificação'))?></td><td><?=e((string)($extension['group']?:'—'))?></td><td><?=e((string)($extension['area_code']?:'—'))?></td><td><?=e((string)($extension['external_caller_id']?:'—'))?></td><td><span class="tdb-badge <?=$extension['recording']?'on':'off'?>"><?=$extension['recording']?'Ativada':'Desativada'?></span></td><td><span class="tdb-badge <?=$extension['blocked']?'blocked':'on'?>"><?=$extension['blocked']?'Bloqueado':'Disponível'?></span></td></tr><?php endforeach;?></tbody></table></div></section><?php endif;?>
    <?php render_commercial_activity_dialog(['user'=>$u,'types'=>$activityTypes??[],'channels'=>$activityChannels??[],'categories'=>$activityCategories??[],'assignable'=>$activityAssignableUsers??[],'return_to'=>'baldussi','return_query'=>'']);?>
   </section>
  <?php break;
  case 'test_data':?>
...
function layout(string $body,?array $u,string $page=''): void{
       ...
       <section class="tdq-card">
        ...
        <div class="tdq-contact-primary">
         ...
         <div class="tdq-contact-lines">
          <div class="tdq-contact-phone"><span><i class="fa-solid fa-phone"></i><b data-client-quick-phone>Telefone não informado</b></span><button type="button" data-client-quick-call-inline title="Ligar pelo CRM" aria-label="Ligar pelo CRM"><i class="fa-solid fa-phone-volume"></i></button></div>
 ...
 <dialog class="tdq-activity-modal" data-client-quick-activity-modal>
  ...
  <dialog class="tdcrm-task-modal" data-global-task-modal>
   <form data-global-task-form>
    ...
    <input type="hidden" name="crm_account_code" data-task-account-code>
    ...
    <div class="tdcrm-task-body">
     ...
     <label class="wide"><span>Descrição</span><textarea class="form-control" name="title" rows="5" maxlength="2000" placeholder="Descreva o que deve ser feito, com detalhes se necessário" required></textarea></label>
 ...
 <?php $floatingCall=is_array($_SESSION['baldussi_test_state']['last_call']??null)?$_SESSION['baldussi_test_state']['last_call']:null;if($floatingCall&&!empty($floatingCall['active'])&&!empty($floatingCall['ok'])&&(int)($floatingCall['requested_by']??0)===(int)($u['id']??0)):$floatingStartedAt=strtotime((string)($floatingCall['started_at']??$floatingCall['at']??'now'));$floatingActivityNotes='Ligação iniciada pelo CRM em '.date('d/m/Y',$floatingStartedAt).' às '.date('H:i',$floatingStartedAt).', via telefonia Baldussi (ramal '.(string)$floatingCall['origin'].').';?>
  <aside class="tdb-call-dock" data-baldussi-call-dock data-started-at="<?=e((string)$floatingStartedAt)?>" data-finish-url="<?=APP_URL?>/settings/baldussi/finish-tracking" data-account-code="<?=e((string)($floatingCall['crm_account_code']??''))?>" data-account-name="<?=e((string)($floatingCall['account_name']??'Cliente'))?>" data-activity-notes="<?=e($floatingActivityNotes)?>" aria-live="polite">
   ...
   <div class="tdb-call-dock-body">
    <span class="tdb-call-dock-icon"><i class="fa-solid fa-phone-volume"></i></span>
    <div class="tdb-call-dock-main"><small>PABX acionado pelo CRM</small><strong><?=e((string)$floatingCall['destination_masked'])?></strong><span>Ramal <?=e((string)$floatingCall['origin'])?> · <?=e((string)($floatingCall['route_label']??'Ligação externa'))?></span></div>
