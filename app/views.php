    ...
    <section class="tdc-table-card">
     ...
     <section class="tdc-bulk" data-client-bulk data-endpoint="<?=APP_URL?>/api/clients/bulk" data-csrf="<?=CSRF::token()?>" data-segment="<?=e($clientSegment)?>">
      ...
      <div class="tdc-bulk-controls">
       <label><span><b>1</b> Alterar vendedor principal</span><select class="form-select" data-client-bulk-seller><option value="">Não alterar principal</option><option value="__none__">Deixar principal sem vendedor</option><?php foreach($bulkSellers??$portfolioSellers??[] as $seller):?><option value="<?=e($seller['omie_code'])?>"><?=e($seller['name'])?></option><?php endforeach;?></select><small>Esta é uma alteração definitiva do cadastro, diferente da carteira provisória do mês.</small></label>
     ...
     <div class="table-card tdc-table-wrap">
      <?php $clientDataParams=['uf'=>$uf,'segment'=>$clientSegment,'month'=>$portfolioMonth];if($ddds)$clientDataParams['ddds']=$ddds;if($tag!=='')$clientDataParams['tag']=$tag;if($sellerFilter!=='')$clientDataParams['seller_filter']=$sellerFilter;if($portfolioMode)$clientDataParams['portfolio']='mine';elseif(Auth::can('seller')&&($clientScope??'all')==='unassigned')$clientDataParams['scope']='unassigned';?>
