 ...
 switch($name){
  ...
  case 'clients':?>
   ...
   <section class="tdc-page <?=$portfolioMode?'tdc-portfolio-page':''?>">
    ...
    <?php if(Auth::can('seller')):?><div class="tdc-seller-base-note"><i class="fa-solid fa-circle-info"></i><div><strong>Base de clientes sem restrição por tag</strong><small>Use esta tela para localizar qualquer cliente ativo, mesmo que esteja classificado na tag errada. “Minha carteira” continua mostrando somente seus clientes. Você pode abrir e corrigir individualmente tags e vínculo quando necessário.</small></div></div><?php endif;?>
    <?php endif;?>
    ...
    <section class="tdc-table-card">
     ...
     <div class="table-card tdc-table-wrap">
      ...
      <table class="table tdc-table clients-datatable" data-server-url="<?=APP_URL?>/api/clients/datatable?<?=e(http_build_query($clientDataParams))?>" data-search="<?=e($q)?>" data-page-length="5" data-length-change="1" data-order-column="<?=Auth::can('admin','supervisor')&&$crmStatus==='active'?1:0?>" data-order-direction="asc">
