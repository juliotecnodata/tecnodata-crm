  ...
  case 'clients':?>
   ...
   <section class="tdc-page <?=$portfolioMode?'tdc-portfolio-page':''?>">
    ...
    <section class="tdc-table-card">
     ...
     <?php if(Auth::can('admin','supervisor')&&$crmStatus==='active'):?>
   ...
   <section class="tdc-page">
    ...
    <div class="tdc-detail-grid">
     ...
     <section class="tdc-card">
      ...
      <?php else:?>
       <div class="tdc-contact-locked"><i class="fa-solid fa-user-lock"></i><strong>Atendimento de outra carteira</strong><p>O cliente pertence a <?=e((string)(!empty($crmPortfolioReady)?($crmOwnerName??$crmOwnerRemoteName??'outro responsável'):$effectiveSellerName))?>. O responsável da carteira é controlado pelo CRM Omie.</p><button class="tdc-btn tdc-btn-schedule" type="button" data-global-task-open data-task-client-id="<?=$client['id']?>" data-task-client-name="<?=e($client['name'])?>" data-task-context="sales"><i class="fa-regular fa-calendar-plus"></i>Nova tarefa</button></div>
      <?php endif;?>
     </section>
    </div>
