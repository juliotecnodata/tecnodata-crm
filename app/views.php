...
function render(string $name,array $vars=[]): void{
 ...
 switch($name){
  ...
  case 'commercial_account':
   ...
   $activities=$activities??[];$commercialNotes=$commercialNotes??[];$audit=$audit??[];$returns=$returns??[];$salesHistory=$salesHistory??[];$nextReturn=$nextReturn??null;
   $activityCount=count($activities);$noteCount=count($commercialNotes);$returnCount=count($returns);$salesCount=count($salesHistory);
   $avgInterval=(float)($account['avg_interval_days']??0);
   if(empty($account['last_purchase_at']))$purchaseCycle='Sem histórico';elseif($avgInterval<=0)$purchaseCycle='Esporádico';elseif($avgInterval<=45)$purchaseCycle='Mensal';elseif($avgInterval<=120)$purchaseCycle='Trimestral';elseif($avgInterval<=220)$purchaseCycle='Semestral';elseif($avgInterval<=400)$purchaseCycle='Anual';else $purchaseCycle='Esporádico';
   ...
   $lastContactTs=!empty($account['last_contact_at'])?strtotime((string)$account['last_contact_at']):false;
   $daysSinceContact=$lastContactTs===false?null:max(0,(int)floor((strtotime(date('Y-m-d'))-strtotime(date('Y-m-d',$lastContactTs)))/86400));
   $nextDueTs=!empty($nextReturn['due_at'])?strtotime((string)$nextReturn['due_at']):false;
   $nextLate=$nextDueTs!==false&&$nextDueTs<time();
   $classificationFromRaw=static function(?string $raw): string{
    $data=$raw?json_decode($raw,true):null;if(!is_array($data))return 'Sem classificação';
    $cfc=!empty($data['cfc']);$reseller=!empty($data['reseller']);
    return $cfc&&$reseller?'CFC + Revendedor':($cfc?'CFC':($reseller?'Revendedor':'Sem classificação'));
   };
   $channelMeta=static function(string $code): array{
    return match($code){
     ...
     'other'=>['Outro','fa-solid fa-ellipsis','other'],
     default=>[CommercialActivityService::channelLabel($code),'fa-solid fa-message','other'],
    };
   };
   $renderTimeline=static function(array $rows)use($channelMeta): void{
    foreach($rows as $activity):
     $typeCode=(string)($activity['activity_type']??'contact_completed');$categoryCode=(string)($activity['category_code']??'');$channel=(string)($activity['channel']??'other');
     [$channelLabel,$channelIcon,$channelTone]=$channelMeta($channel);
     if($typeCode==='sale'){$channelLabel='Venda';$channelIcon='fa-solid fa-cart-shopping';$channelTone='sale';}
     $title=$categoryCode!==''?CommercialActivityService::categoryLabel($categoryCode):CommercialActivityService::activityTypeLabel($typeCode);
     ?>
     <article class="tdca-timeline-item <?=e($typeCode)?>">
      <div class="tdca-timeline-rail"><i></i></div>
      <time><?=date('d/m',strtotime((string)$activity['created_at']))?><small><?=date('H:i',strtotime((string)$activity['created_at']))?></small></time>
      <span class="tdca-channel <?=e($channelTone)?>"><i class="<?=e($channelIcon)?>"></i><?=e($channelLabel)?></span>
      <div class="tdca-timeline-copy"><strong><?=e($title)?></strong><p><?=!empty($activity['notes'])?nl2br(e((string)$activity['notes'])):'Atividade registrada sem observação adicional.'?></p><small>Por <?=e((string)($activity['user_name']??'Usuário'))?></small></div>
     </article>
    <?php endforeach;
   };
   ?>
   <section class="tdca-page">
    <a class="tdca-back" href="<?=APP_URL?>/my-portfolio"><i class="fa-solid fa-arrow-left"></i>Voltar para a carteira</a>

    <header class="tdca-hero">
     <div class="tdca-hero-main">
      <span class="tdca-avatar"><i class="fa-regular fa-building"></i></span>
      <div class="tdca-identity">
       <div class="tdca-title-row">
        <h1><?=e($display!==''?$display:'Conta sem nome')?></h1>
        <?php if($isCfc):?><span class="tdca-badge cfc">CFC</span><?php endif;?>
        <?php if($isReseller):?><span class="tdca-badge reseller">Revendedor</span><?php endif;?>
        <?php if(!$isCfc&&!$isReseller):?><span class="tdca-badge neutral">Sem classificação</span><?php endif;?>
        <?php if($canWork):?><button class="tdca-edit-class" type="button" data-tdca-classification-open><i class="fa-solid fa-pen"></i>Editar</button><?php endif;?>
       </div>
       <div class="tdca-document"><?=e((string)($account['document']?:'Documento não informado'))?></div>
       <div class="tdca-meta">
     ...
     <div class="tdca-hero-actions">
      <div class="tdca-health">
       <span class="<?=$nextLate?'late':'ok'?>"><i class="fa-solid <?=$nextLate?'fa-circle-exclamation':'fa-circle-check'?>"></i><?=$nextLate?'Retorno atrasado':'Em dia'?></span>
       <span class="contact"><i class="fa-regular fa-calendar-check"></i><?=$daysSinceContact===null?'Sem contato registrado':$daysSinceContact.' dia'.($daysSinceContact===1?'':'s').' desde o último contato'?></span>
      </div>
      <?php if($canWork):?><button class="tdca-new-activity" type="button" data-commercial-activity-open data-activity-type="contact_completed" data-account-code="<?=e((string)$account['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($account['owner_name']?:$u['name']))?>" data-account-type="<?=e($classificationLabel)?>" data-client-id="<?=$linked?(int)$account['client_id']:''?>"><i class="fa-solid fa-plus"></i>Nova atividade</button><?php endif;?>
     </div>
    </header>

    <?php if($flash):?><div class="alert alert-<?=e($flash['type']??'success')?>"><?=e($flash['message']??'')?></div><?php endif;?>

    <nav class="tdca-tabs" aria-label="Ficha do cliente">
     <button type="button" class="active" data-tdca-tab="overview"><i class="fa-solid fa-table-cells-large"></i>Visão geral</button>
     <button type="button" data-tdca-tab="activities"><i class="fa-regular fa-file-lines"></i>Atividades <b><?=$activityCount?></b></button>
     <button type="button" data-tdca-tab="returns"><i class="fa-regular fa-clock"></i>Retornos <b><?=$returnCount?></b></button>
     <button type="button" data-tdca-tab="sales"><i class="fa-solid fa-chart-column"></i>Vendas <b><?=$salesCount?></b></button>
     <button type="button" data-tdca-tab="observations"><i class="fa-regular fa-note-sticky"></i>Observações <b><?=$noteCount?></b></button>
    </nav>

    <div class="tdca-panel active" data-tdca-panel="overview">
     <div class="tdca-overview-grid">
      <section class="tdca-card tdca-history">
       ...
       <div class="tdca-timeline">
        <?php $renderTimeline(array_slice($activities,0,8));?>
        <?php if(!$activities):?><div class="tdca-empty"><i class="fa-regular fa-comments"></i><strong>Nenhuma atividade registrada</strong><span>Use “Nova atividade” para iniciar o histórico deste cliente.</span></div><?php endif;?>
       </div>
       <?php if(count($activities)>8):?><footer><button type="button" data-tdca-open-tab="activities">Ver histórico completo</button></footer><?php endif;?>
      </section>

      <aside class="tdca-side">
       <section class="tdca-card tdca-next <?=$nextLate?'late':''?>">
        <header><span><i class="fa-regular fa-calendar"></i></span><strong>Próxima ação</strong></header>
        <?php if($nextReturn&&$nextDueTs!==false):?>
         <h3><?=$nextLate?'Retorno atrasado':'Retorno agendado'?> <?=date('d/m',$nextDueTs)?> às <?=date('Hh',$nextDueTs)?></h3>
         <p><?=e(preg_replace('/^Retorno\s*·\s*/u','',(string)($nextReturn['title']??'Próximo contato com o cliente.')))?></p>
         <small>Responsável: <?=e((string)($nextReturn['assigned_name']??'Não informado'))?></small>
         <?php if($canWork):?><div class="tdca-next-actions">
          <form method="post" action="<?=APP_URL?>/commercial/accounts/<?=rawurlencode((string)$account['omie_code'])?>/returns/<?=(int)$nextReturn['id']?>/complete"><input type="hidden" name="_token" value="<?=CSRF::token()?>"><button class="primary" type="submit"><i class="fa-solid fa-check"></i>Concluir retorno</button></form>
        ...
        <?php else:?>
         <h3>Nenhum retorno agendado</h3><p>Não há compromisso comercial pendente para esta conta.</p>
         <?php if($canWork):?><button type="button" class="tdca-next-single" data-commercial-activity-open data-activity-type="follow_up" data-account-code="<?=e((string)$account['omie_code'])?>" data-account-name="<?=e($display)?>" data-account-owner="<?=e((string)($account['owner_name']?:$u['name']))?>" data-account-type="<?=e($classificationLabel)?>" data-client-id="<?=$linked?(int)$account['client_id']:''?>">Registrar atividade</button><?php endif;?>
        <?php endif;?>
       </section>

       <section class="tdca-card tdca-observation-card">
        <header><span><i class="fa-regular fa-clipboard"></i></span><strong>Observações</strong><button type="button" data-tdca-open-tab="observations" title="Abrir observações"><i class="fa-solid fa-pen"></i></button></header>
        <?php if(!empty($commercialNotes[0])):?><p><?=nl2br(e((string)$commercialNotes[0]['note']))?></p><small><?=e((string)$commercialNotes[0]['user_name'])?> · <?=date('d/m/Y H:i',strtotime((string)$commercialNotes[0]['created_at']))?></small><?php else:?><p class="muted">Nenhuma observação livre registrada para esta conta.</p><?php endif;?>
    ...
    <div class="tdca-panel" data-tdca-panel="activities">
     <section class="tdca-card tdca-full-card">
      <header><div><strong>Atividades</strong><small>Histórico completo do relacionamento comercial.</small></div><span><?=$activityCount?> registro<?=$activityCount===1?'':'s'?></span></header>
      <div class="tdca-timeline"><?php $renderTimeline($activities);?><?php if(!$activities):?><div class="tdca-empty"><i class="fa-regular fa-comments"></i><strong>Nenhuma atividade registrada</strong></div><?php endif;?></div>
     </section>
    </div>

    <div class="tdca-panel" data-tdca-panel="returns">
     <section class="tdca-card tdca-full-card">
      <header><div><strong>Retornos</strong><small>Compromissos comerciais vinculados a esta conta.</small></div><span><?=$returnCount?> retorno<?=$returnCount===1?'':'s'?></span></header>
      <div class="tdca-return-list">
       <?php foreach($returns as $task):$due=strtotime((string)$task['due_at']);$status=(string)$task['status'];$late=$status==='pending'&&$due!==false&&$due<time();?>
        <article>
         <span class="tdca-return-date"><b><?=date('d/m',$due)?></b><small><?=date('H:i',$due)?></small></span>
         <div><strong><?=e((string)$task['title'])?></strong><small>Responsável: <?=e((string)($task['assigned_name']??'Não informado'))?></small></div>
         <span class="tdca-return-status <?=$status?> <?=$late?'late':''?>"><?=$late?'Atrasado':($status==='pending'?'Pendente':($status==='done'?'Concluído':'Cancelado'))?></span>
        </article>
       <?php endforeach;?>
       <?php if(!$returns):?><div class="tdca-empty"><i class="fa-regular fa-calendar-xmark"></i><strong>Nenhum retorno registrado</strong></div><?php endif;?>
      </div>
     </section>
    </div>

    <div class="tdca-panel" data-tdca-panel="sales">
     <section class="tdca-card tdca-full-card">
      <header><div><strong>Vendas</strong><small>Pedidos e serviços localizados para o cliente vinculado.</small></div><span><?=$salesCount?> registro<?=$salesCount===1?'':'s'?></span></header>
      <div class="tdca-sales-list">
       <?php foreach($salesHistory as $sale):$saleDate=!empty($sale['sale_date'])?strtotime((string)$sale['sale_date']):false;?>
        <article>
         <span class="tdca-sale-icon"><i class="fa-solid <?=$sale['source_type']==='service'?'fa-screwdriver-wrench':'fa-cart-shopping'?>"></i></span>
         <div><strong><?=$sale['source_type']==='service'?'Serviço':'Pedido'?> <?=e((string)($sale['reference_number']?:$sale['omie_code']))?></strong><small><?=$saleDate?date('d/m/Y',$saleDate):'Data não informada'?> · <?=e((string)($sale['status']?:'Status não informado'))?></small></div>
         <b><?=money($sale['total']??0)?></b>
        </article>
       <?php endforeach;?>
       <?php if(!$salesHistory):?><div class="tdca-empty"><i class="fa-solid fa-cart-shopping"></i><strong>Nenhuma venda localizada</strong><span><?=$linked?'Não há pedidos ou serviços sincronizados para este cliente.':'A conta ainda não está vinculada ao Cliente Geral.'?></span></div><?php endif;?>
      </div>
     </section>
    </div>

    <div class="tdca-panel" data-tdca-panel="observations">
     <section class="tdca-card tdca-full-card">
      <header><div><strong>Observações</strong><small>Contexto livre para consulta da equipe. Não altera prioridade nem conta como atividade.</small></div><span><?=$noteCount?> nota<?=$noteCount===1?'':'s'?></span></header>
      <?php if($canWork):?><form class="tdca-note-form" method="post" action="<?=APP_URL?>/commercial/accounts/<?=rawurlencode((string)$account['omie_code'])?>/note"><input type="hidden" name="_token" value="<?=CSRF::token()?>"><textarea name="note" rows="3" maxlength="10000" placeholder="Registre uma observação interna sobre este cliente..." required></textarea><button type="submit"><i class="fa-solid fa-plus"></i>Adicionar observação</button></form><?php endif;?>
      <div class="tdca-notes-list">
       ...
       <?php if(!$commercialNotes):?><div class="tdca-empty"><i class="fa-regular fa-note-sticky"></i><strong>Nenhuma observação registrada</strong></div><?php endif;?>
      </div>
     </section>
    </div>

    <?php if($canWork):?>
    <dialog class="tdca-classification-dialog" data-tdca-classification-dialog>
     <form method="post" action="<?=APP_URL?>/commercial/accounts/<?=rawurlencode((string)$account['omie_code'])?>/profile">
      ...
      <header><div><strong>Classificação do cliente</strong><small>Alterações ficam registradas na trilha de auditoria.</small></div><button type="button" data-tdca-classification-close aria-label="Fechar"><i class="fa-solid fa-xmark"></i></button></header>
      <div class="tdca-classification-body">
       <div class="tdca-class-options">
       ...
       <section class="tdca-audit-trail"><h3>Histórico da classificação</h3>
        <?php $classificationAudit=array_values(array_filter($audit,static fn($change)=>(string)($change['field_name']??'')==='classification'));foreach(array_slice($classificationAudit,0,8) as $change):?>
         <p><b><?=date('d/m',strtotime((string)$change['created_at']))?></b>, <?=e((string)($change['actor_name']??'Sistema'))?> alterou de <strong><?=e($classificationFromRaw($change['previous_value']??null))?></strong> para <strong><?=e($classificationFromRaw($change['new_value']??null))?></strong>.</p>
        <?php endforeach;?>
        <?php if(empty($classificationAudit)):?><p class="muted">Ainda não há alterações manuais registradas.</p><?php endif;?>
       </section>
      </div>
      <footer><button type="button" class="secondary" data-tdca-classification-close>Cancelar</button><button type="submit" class="primary"><i class="fa-solid fa-floppy-disk"></i>Salvar classificação</button></footer>
     </form>
    </dialog>
    <?php endif;?>
   ...
   <section class="tdc-page <?=$portfolioMode?'tdc-portfolio-page':''?>">
    ...
    <section class="tdc-table-card">
     ...
     <div class="table-card tdc-table-wrap">
      <?php $clientDataParams=['ufs'=>$clientUfs,'segment'=>$clientSegment,'month'=>$portfolioMonth,'crm_status'=>$crmStatus,'classification'=>$classificationFilter];if($ddds)$clientDataParams['ddds']=$ddds;if($clientTagsSelected)$clientDataParams['tags']=$clientTagsSelected;if($sellerFilter!=='')$clientDataParams['seller_filter']=$sellerFilter;if($portfolioMode)$clientDataParams['portfolio']='mine';elseif(Auth::can('seller')&&($clientScope??'all')==='unassigned')$clientDataParams['scope']='unassigned';?>
