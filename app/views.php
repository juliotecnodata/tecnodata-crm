...
function render(string $name,array $vars=[]): void{
 ...
 switch($name){
  ...
  case 'client_audit':
   ...
   $auditUf=$auditUf??'';$auditSeller=$auditSeller??'';$auditSource=$auditSource??'all';$auditMinRecords=$auditMinRecords??2;$auditDateFrom=$auditDateFrom??'';$auditDateTo=$auditDateTo??'';$auditSort=$auditSort??'records_desc';$auditUfs=$auditUfs??[];$auditSellers=$auditSellers??[];
   $auditUrl=static function(array $changes=[])use($auditTab,$auditQuery,$auditConflict,$auditMonth,$auditUf,$auditSeller,$auditSource,$auditMinRecords,$auditDateFrom,$auditDateTo,$auditSort){$params=['tab'=>$auditTab,'q'=>$auditQuery,'conflict'=>$auditConflict,'month'=>$auditMonth,'uf'=>$auditUf,'seller'=>$auditSeller,'source'=>$auditSource,'min_records'=>$auditMinRecords,'date_from'=>$auditDateFrom,'date_to'=>$auditDateTo,'sort'=>$auditSort];foreach($changes as $key=>$value){if($value===null||$value===''||$value==='all'||($key==='min_records'&&(int)$value===2)||($key==='sort'&&$value==='records_desc'))unset($params[$key]);else $params[$key]=$value;}return APP_URL.'/clients-audit?'.http_build_query($params);};
   ?>
   <section class="tdaudit-page">
    ...
    <?php if(in_array($auditTab,['duplicates','inactive'],true)):?><dialog class="tdc-omie-check-modal tdaudit-omie-modal" data-client-omie-modal data-audit-omie-modal data-client-id="0" data-csrf="<?=e(CSRF::token())?>">
     ...
     <footer><span data-client-omie-summary></span><div><button class="tdc-btn" type="button" data-client-omie-close>Fechar</button><button class="tdc-btn tdc-btn-danger" type="button" data-client-omie-batch-delete disabled><i class="fa-solid fa-trash-can"></i>Excluir selecionados do CRM</button><button class="tdc-btn tdaudit-batch-inactivate" type="button" data-client-omie-batch-inactivate disabled><i class="fa-solid fa-user-slash"></i>Inativar selecionados + excluir CRM</button><button class="tdc-btn tdc-btn-primary" type="button" data-client-omie-reconcile><i class="fa-solid fa-arrows-rotate"></i>Aplicar situação da Omie no CRM</button></div></footer>
    </dialog><?php endif;?>
    ...
    <form class="tdaudit-filter <?=$auditTab==='duplicates'?'duplicates':''?>" method="get" action="<?=APP_URL?>/clients-audit">
     ...
     <?php $hasAuditFilter=$auditQuery!==''||$auditConflict!=='all'||$auditUf!==''||$auditSeller!==''||$auditSource!=='all'||(int)$auditMinRecords!==2||$auditDateFrom!==''||$auditDateTo!==''||$auditSort!=='records_desc';if($hasAuditFilter):?><a class="tdaudit-btn" href="<?=APP_URL?>/clients-audit?<?=e(http_build_query(['tab'=>$auditTab,'month'=>$auditMonth]))?>"><i class="fa-solid fa-xmark"></i>Limpar</a><?php endif;?></div>
    </form>
