   ...
   <section class="tdc-page <?=$portfolioMode?'tdc-portfolio-page':''?>">
    ...
    <section class="tdc-table-card">
     ...
     <div class="table-card tdc-table-wrap">
      <?php $clientDataParams=['ufs'=>$clientUfs,'segment'=>$clientSegment,'month'=>$portfolioMonth,'crm_status'=>$crmStatus];if($ddds)$clientDataParams['ddds']=$ddds;if($clientTagsSelected)$clientDataParams['tags']=$clientTagsSelected;if($sellerFilter!=='')$clientDataParams['seller_filter']=$sellerFilter;if($portfolioMode)$clientDataParams['portfolio']='mine';elseif(Auth::can('seller')&&($clientScope??'all')==='unassigned')$clientDataParams['scope']='unassigned';?>
    ...
    <section class="tdc-card tdc-commercial-audit">
     ...
     <div class="tdc-commercial-audit-list">
      <?php foreach(array_slice($commercialAudit,0,15) as $change):$field=(string)($change['field_name']??'');$old=json_decode((string)($change['previous_value']??''),true);$new=json_decode((string)($change['new_value']??''),true);?>
       ...
       <?php if($field==='classification'&&is_array($old)&&is_array($new)):?><p><?=(!empty($old['cfc'])?'CFC ':'').(!empty($old['reseller'])?'Revendedor':'Sem classificação')?> <i class="fa-solid fa-arrow-right"></i> <?=(!empty($new['cfc'])?'CFC ':'').(!empty($new['reseller'])?'Revendedor':'Sem classificação')?></p>
