...
CREATE TABLE IF NOT EXISTS sync_outbox(id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,entity_type VARCHAR(50) NOT NULL,entity_id VARCHAR(100) NOT NULL,operation VARCHAR(50) NOT NULL,payload_json JSON NOT NULL,status ENUM('pending','processing','synced','error','ignored') NOT NULL DEFAULT 'pending',attempts INT UNSIGNED NOT NULL DEFAULT 0,last_error TEXT NULL,next_attempt_at DATETIME NULL,created_at DATETIME NOT NULL,updated_at DATETIME NOT NULL,synced_at DATETIME NULL,UNIQUE KEY uq_sync_outbox_event(entity_type,entity_id,operation,status),INDEX idx_sync_outbox_status(status,next_attempt_at,id));
